...
struct napi_struct {
	...
	int			(*poll)(struct napi_struct *, int);                /*钩子函数收包process_msg*/
...
/*
  描述每一个网络设备
  net_device结构则向内核提供了操作网卡的抽象接口
*/
struct net_device {
	char			name[IFNAMSIZ];                                /*用于存放网络设备的设备名称*/
	struct hlist_node	name_hlist;                                /*网络设备的别名*/
	...
	struct list_head	napi_list;                                /*设备的napi_struct 链*/
	...
	unsigned short		type;                     /*接口硬件类型，在<if_arp.h>中定义了每一个接口硬件类型；*/
	...
	struct netdev_hw_addr_list	dev_addrs;        /*ist of device hw address;*/
 ...
 */
	...
	unsigned char		broadcast[MAX_ADDR_LEN];                    /*hw broadcast address*/
#ifdef CONFIG_RFS_ACCEL
...
#endif
	unsigned int		tx_queue_len;                              /*Max frame per queue allowned*/
