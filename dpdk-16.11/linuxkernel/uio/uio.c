...
/*uio 中断监听结构，每个uio设备一个结构，跟踪uio设备中断*/
struct uio_listener {
	struct uio_device *dev;       /*uio 设备*/
	s32 event_count;              /*中断事件计数*/
};
...
调用模块：init_uio_class() 
执行流程： 申请字符设备号，设备，并注册到系统中，注册uio_class到系统中 //init_uio_class 
//创建”/sys/class/uio”*/
			  ...
			  struct uio_info *info)
{
	...
	/*注册uio到设备，生成代表uio设备的系统文件/sys/class/uio/uiox*/
	/*/sys/class/uio/uiox/maps/map0 下存在addr、name、offset、 size，已映射到用户空间的内存*/
	/*在用户空间访问addr*/
