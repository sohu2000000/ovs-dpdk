...
static const unsigned int tcp_timeouts[TCP_CONNTRACK_TIMEOUT_MAX] = {
	[TCP_CONNTRACK_SYN_SENT]	= 2 MINS,		/*SYNµÈ´ý2·ÖÖÓ*/
	[TCP_CONNTRACK_SYN_RECV]	= 60 SECS,
	[TCP_CONNTRACK_ESTABLISHED]	= 5 DAYS,		/*Á´½ÓÎ¬³Ö5Ìì*/
...
/* RFC1122 says the R2 limit should be at least 100 seconds.
   ...
   to ~13-30min depending on RTO. */
	[TCP_CONNTRACK_RETRANS]		= 5 MINS,		/*ÖØ´«5·ÖÖÓ*/
...
/*tcpÉèÖÃÎ»*/
enum tcp_bit_set {
	TCP_SYN_SET,		/*syn±ê¼ÇÎ»*/
	TCP_SYNACK_SET,		/*syn-ack±ê¼ÇÎ»*/
	TCP_FIN_SET,	    /*FIN±ê¼ÇÎ»*/
	TCP_ACK_SET,		/*ack±ê¼ÇÎ»*/
	TCP_RST_SET,		/*rst±ê¼ÇÎ»*/
	TCP_NONE_SET,		/*·Çtcp±ê¼ÇÎ»*/
};
...
/*******************************************************************************
 º¯ÊýÃû³Æ :  tcp_pkt_to_tuple
 ¹¦ÄÜÃèÊö :  skbÔ´¡¢Ä¿¶Ë¿ÚÌî³ätuple
 ÊäÈë²ÎÊý :  skb---skb±¨ÎÄ
 			 dataoff---4²ãÆ«ÒÆ
 			 net---namespace
 			 tuple---´æ´¢tuple
...
/*******************************************************************************
 º¯ÊýÃû³Æ :  tcp_invert_tuple
 ¹¦ÄÜÃèÊö :  ·´Ïòtuple¶Ë¿Ú¹¹½¨
 ÊäÈë²ÎÊý :  inverse---·´Ïòtuple
 			 orig---ÕýÏòtuple
 Êä³ö²ÎÊý :  ÎÞ
 ·µ»ØÖµ¡¡ :  ÎÞ
 --------------------------------------------------------------------------------
...
/*******************************************************************************
 º¯ÊýÃû³Æ :  get_conntrack_index
 ¹¦ÄÜÃèÊö :  ·µ»Øµ±Ç°±¨ÎÄtcp flag
 ÊäÈë²ÎÊý :  
 Êä³ö²ÎÊý :  ÎÞ
 ·µ»ØÖµ¡¡ :  ÎÞ
 --------------------------------------------------------------------------------
...
<<<<<<< HEAD
/*******************************************************************************
 º¯ÊýÃû³Æ :  tcp_options
 ¹¦ÄÜÃèÊö :   tcp Ñ¡ÏîÉèÖÃ
 ÊäÈë²ÎÊý :  ct---nf_conn½á¹¹£¬Ò»ÌõÁ´½Ó¸ú×ÙÏî
=======

/*******************************************************************************
 º¯ÊýÃû³Æ :  tcp_options
 ¹¦ÄÜÃèÊö :  tcpÑ¡ÏîÌî³ä
 ÊäÈë²ÎÊý :  skb---µ±Ç°±¨ÎÄÎªSYN±¨ÎÄ
 			 dataoff---4²ã±¨ÎÄÆ«ÒÆ
 			 tcph---4²ãÍ·µØÖ·
 			 state--ct tcp×´Ì¬
...
/*******************************************************************************
 º¯ÊýÃû³Æ :  tcp_in_window
 ¹¦ÄÜÃèÊö :  ÅÐ¶ÏÒ»¸öTCP°üÐòÁÐºÅºÍÈ·ÈÏºÅÊÇ·ñÔÚ¸ø¶¨window·¶Î§ÄÚµÄº¯ÊýÊÇtcp_in_windowÚ
 ÊäÈë²ÎÊý :  SACK--ÔÊÐíÈ·ÈÏ²»Á¬ÐøµÄtcp±¨ÎÄ
 Êä³ö²ÎÊý :  ct---µ±Ç°Á´½Ó¸ú×Ù
 			 state---ctµÄtcp×´Ì¬
 			 dir---·½Ïò origin
 			 index---µ±Ç°±¨ÎÄµÄtcp flag
 			 skb---µ±Ç°±¨ÎÄskb
 			 dataoff---µ±Ç°±¨ÎÄ4²ãÆ«ÒÆ
 			 tcph---tcpÍ·
 ·µ»ØÖµ¡¡ :  ÎÞ
 --------------------------------------------------------------------------------
 ×î½üÒ»´ÎÐÞ¸Ä¼ÇÂ¼ :
			...
			unsigned int index, const struct sk_buff *skb, unsigned int dataoff, const struct tcphdr *tcph)
{
	...
	// ¿Í»§¶Ë·¢µÄµÚÒ»¸öSYN°üÊÇµ½²»ÁËÕâ¸öº¯ÊýµÄ,Ö±½Ó¾Í½ÓÊÜÁË,
	// ÊÇ´ÓÁ¬½ÓµÄµÚ2¸ö°üÒÔºó²Å½øÈë±¾º¯Êý´¦Àí
	...
	/*±¾Êý¾Ý°ü½áÊøÐòÁÐºÅ£¬±¾±¨ÎÄÐòºÅ+±¨ÎÄ³¤¶È*/
	end = segment_seq_plus_len(seq, skb->len, dataoff, tcph);

	/*tcp¿ªÆôÁËSACKÔÊÐí¹¦ÄÜ*/
	if (receiver->flags & IP_CT_TCP_FLAG_SACK_PERM)
		tcp_sack(skb, dataoff, tcph, &sack);

	/* Take into account NAT sequence number mangling */
	/*½ÓÊÕ·½Ö§³ÖSACKµÄ»°¼ì²éÊÇ·ñÔÚTCPÑ¡ÏîÖÐÓÐSACK*/
	...
	if (sender->td_maxwin == 0) 
	{
		...
		if (tcph->syn) 
		{
			...
			/*·¢ËÍ·½ÐòÁÐºÅºÍ±¨ÎÄ³¤¶ÈÖ®ºÍ×î´óÖµ*/
			sender->td_end = sender->td_maxend = end;

			/*·¢ËÍ·½»¬¶¯´°¿Ú×î´óÖµ*/
			sender->td_maxwin = (win == 0 ? 1 : win);

			/*·¢ËÍ·½¼ì²éTCPÑ¡Ïî£¬ÅÐ¶Ï½ÓÊÕ·½ÊÇ·ñÖ§³ÖSACKºÍ´°¿ÚÀ©´ó*/
	...
	/*original·½Ïò¿´µ½ÁËSYN±¨ÎÄ¡¢reply·½Ïò¿´µ½SYN ACK±¨ÎÄ*/
	...
	/*·ÇACK°üºÍRST°ü,½«È·ÈÏºÅÖÃÎª½ÓÊÕ·½µÄ½áÊøÐòÁÐºÅ*/
	if (!(tcph->ack)) 
	{
		/*
	...
	else 
	{
		...
		/*¶Ô·Ç·¨°üµÄÈ±Ê¡²ßÂÔ,0¾Ü¾ø,·Ç0½ÓÊÜ.¸Ã²ÎÊý¿ÉÍ¨¹ý/procÎÄ¼þÏµÍ³ÉèÖÃ*/
...
/*******************************************************************************
 º¯ÊýÃû³Æ :  tcp_error
 ¹¦ÄÜÃèÊö :  Ö÷Òª¼ì²éÁËtcp±¨ÎÄÍ·µÄÍêÕûÐÔ£¬Ð£ÑéºÍµÄÕýÈ·ÐÔÒÔ¼° flagµÄÓÐÐ§ÐÔ
 ÊäÈë²ÎÊý :  
 Êä³ö²ÎÊý :  ÎÞ
 ·µ»ØÖµ¡¡ :  ÎÞ
 --------------------------------------------------------------------------------
...
/*******************************************************************************
 º¯ÊýÃû³Æ :  nf_conntrack_init_net
 ¹¦ÄÜÃèÊö :  Í¨¹ýÁ¬½Óµ±Ç°µÄ×´Ì¬£¬µ½´ïµÄÐÂ±¨ÎÄ£¬µÃµ½Á¬½ÓÐÂµÄ×´Ì¬²¢½øÐÐ¸üÐÂ£¬
 			 ÆäÊµ¾ÍÊÇÒ»´Î²éÑ¯£¬ÊäÈëÊÇ·½Ïò+±¨ÎÄÐÅÏ¢+¾É×´Ì¬£¬Êä³öÊÇÐÂ×´Ì¬
 ÊäÈë²ÎÊý :  ct---nf_conn
 			 skb---skb±¨ÎÄ
 			 dataoff---4²ãÊý¾Ý²¿·Ö¿ªÊ¼
 			 ctinfo---Á´½Ó×´Ì¬Öµ
 			 timeouts---³¬Ê±Ê±¼ä
...
static int tcp_packet(struct nf_conn *ct, const struct sk_buff *skb, unsigned int dataoff, enum ip_conntrack_info ctinfo, unsigned int *timeouts)
{
	...
	/*»ñÈ¡Á´½Ó¶ÔÓ¦·½ÏòµÄtupleÖµ*/
	tuple = &ct->tuplehash[dir].tuple;

	/*ÐÂ×´Ì¬×ª»»*/
	switch (new_state) 
	{
		/*µ±Ç°±¨ÎÄÎªSYN±¨ÎÄ£¬ÐÂ×´Ì¬ÎªSYN_SENT*/
		case TCP_CONNTRACK_SYN_SENT:
		{
			/*tcpÐ­ÒéµÄ×´Ì¬Ð¡ÓÚ4´Î»ÓÊÖ£¬ÎªÊ²Ã´£¬¾É×´Ì¬±ØÐëÎª·Ç½»»¥×´Ì¬*/
			...
			/*ÖØÐÂÁ´½Ó£¬Ôò¶ªÆú±¾syn±¨ÎÄ£¬É¾³ýct£¬ÎªÊ²Ã´*/
			/*Õý·´ÏòflagÉèÖÃÁËIP_CT_TCP_FLAG_CLOSE_INIT »ò ×îºóÒ»¸ö±¨ÎÄÎªdir·½Ïò¡¢»ò×îºóÒ»¸ö±¨ÎÄÎªTCP_RST_SET*/
			if (

				/*2¸ö·½Ïò¾ÉµÄtcp flag´æÔÚÉèÖÃÁËFIN ±¨ÎÄ*/
				((ct->proto.tcp.seen[dir].flags | ct->proto.tcp.seen[!dir].flags) & IP_CT_TCP_FLAG_CLOSE_INIT)

				/*×îºóÒ»¸ö°üµÄ·½ÏòÊÇorigin·½Ïò*/
			   ...
			   ) 
			{
				...
				/*ÕâÖÖÇé¿ödropµôÕâ¸öSYN±¨ÎÄ*/
				return NF_DROP;
			}
			/* Fall through */
		}

		/*µ±Ç°±¨ÎÄÎªSYNACK±¨ÎÄ¡¢ÐÂ×´Ì¬¸ÄÎªTCP_CONNTRACK_SYN_RECV£¬ÕâÐ©±¨ÎÄ¶¼ÊÇºöÂÔtcpÁ´½Ó¸ú×Ù±ê¼Çºê*/
		case TCP_CONNTRACK_IGNORE:
		{
			...
			/*µ±Ç°ÊÇreply·½ÏòµÄsyn ack ±¨ÎÄ¡¢ÉÏÒ»¸ö±¨ÎÄÎªSYN±¨ÎÄ¡¢ÐòÁÐºÅ¼ÓlenÐ£ÑéºÏ·¨*/
			    ...
			    && ntohl(th->ack_seq) == ct->proto.tcp.last_end) 
			{
				...
				/*ÉèÖÃ¾É×´Ì¬Îª·¢ÁËSYN±¨ÎÄ×´Ì¬*/
				/*nf×´Ì¬SYN_SENT¸ÄÎªSYN_RECV*/
				old_state = TCP_CONNTRACK_SYN_SENT;
				new_state = TCP_CONNTRACK_SYN_RECV;

				/*¸üÐÂµ±Ç°·½Ïò(reply)µÄtcp×´Ì¬*/
				/*¼ÇÂ¼ÉÏÒ»¸ö±¨ÎÄµÄÐòÁÐºÅºÍ±¨ÎÄ³¤¶ÈÖ®ºÍ×î´óÖµ*/
				ct->proto.tcp.seen[ct->proto.tcp.last_dir].td_end = ct->proto.tcp.last_end;

				/*¼ÇÂ¼ÉÏÒ»¸ö±¨ÎÄ ack+»¬¶¯´°¿Ú×î´óÖµ*/
			...
			/*µ±Ç°±¨ÎÄµÄtcp flag ¸³Öµ¸øtcp½á¹¹*/
			ct->proto.tcp.last_index = index;

			/*µ±Ç°±¨ÎÄµÄ·½Ïò¸³Öµ¸øtcp½á¹¹*/
			ct->proto.tcp.last_dir = dir;

			/*¼ÇÂ¼µ±Ç°±¨ÎÄµÄÐòÁÐºÅµ½tcp½á¹¹*/
			ct->proto.tcp.last_seq = ntohl(th->seq);

			/*µ±Ç°±¨ÎÄµÄÐòÁÐºÅ¼Ólen¼ÇÂ¼µ½tcp½á¹¹*/
			ct->proto.tcp.last_end = segment_seq_plus_len(ntohl(th->seq), skb->len, dataoff, th);

			/*µ±Ç°±¨ÎÄµÄtcp»¬¶¯´°¿Ú¼ÇÂ¼µ½tcp½á¹¹*/
			...
			/*µ±Ç°±¨ÎÄÎªsyn±¨ÎÄ¡¢original·½Ïò*/
			if (index == TCP_SYN_SET && dir == IP_CT_DIR_ORIGINAL) 
			{
				struct ip_ct_tcp_state seen = {};

				/*Çå¿Õtcp¼ÇÂ¼µÄtcp flag¡¢last_wscale*/
				...
				{
					/*¸³Öµtcp flag¡¢last_wscale*//*»¬¶¯´°¿Ú±»·¢ËÍÕß¸æÖª*/
		...
		case TCP_CONNTRACK_MAX:
		{
			...
			if (
				...
				/*×îºóÒ»¸ö±¨ÎÄÎªorigin·½Ïò£¬origin·½Ïò»ØµÄ×îºóÒ»¸öack*/	
				&& ct->proto.tcp.last_dir == IP_CT_DIR_ORIGINAL 

				/*ÐòÁÐºÅºÍ±¨ÎÄ³¤¶ÈÖ®ºÍ×î´óÖµÐ£Ñé*/
		...
		/*ÐÂ×´Ì¬Îª¼ûµ½ÁËFIN±¨ÎÄ£¬µ±Ç°±¨ÎÄÎªµÚÒ»¸öFIN±¨ÎÄ*/
		/*tcp conntrack*/
		/*4´Î»ÓÊÖµÄACK±¨ÎÄ*/
		case TCP_CONNTRACK_TIME_WAIT:
		{
			...
			if (
				...
				/*µ±Ç°ct¼ûµ½µÄ×îºóÒ»¸ö±¨ÎÄÎªsyn±¨ÎÄ*/
				&& ct->proto.tcp.last_index == TCP_SYN_SET 
				&& (ct->proto.tcp.last_flags & IP_CT_EXP_CHALLENGE_ACK)
				) 
			{
				/* Detected RFC5961 challenge ACK */
				/*ÉÏÒ»¸ö±¨ÎÄ±ê¼ÇÈ¡ÏûIP_CT_EXP_CHALLENGE_ACK*/
				ct->proto.tcp.last_flags &= ~IP_CT_EXP_CHALLENGE_ACK;
				spin_unlock_bh(&ct->lock);
		...
		/*ÐÂ×´Ì¬Îªnf ¼ûµ½ÁË»ØÓ¦µÄsyn ack±¨ÎÄ*/
		...
		case TCP_CONNTRACK_CLOSE:
		{
			...
			/*µ±Ç°±¨ÎÄÎªrst±¨ÎÄ¡¢Îª»ØÓ¦·½Ïò¡¢ÉÏÒ»¸ö±¨ÎÄÎªSYN±¨ÎÄ¡¢Á´½ÓÒÑ½¨Á¢¹ý¡¢ACK±ê¼ÇÒ²ÉèÖÃÁË*/
			if (index == TCP_RST_SET
	...
	/*ÅÐ¶ÏÒ»¸öTCP°üÐòÁÐºÅºÍÈ·ÈÏºÅÊÇ·ñÔÚ¸ø¶¨window·¶Î§ÄÚ*/
...
in_window:
	...
	/*´°¿ÚÄÚµÄ±¨ÎÄ*/
	/*¸üÐÂ¸³Öµ¼ÇÂ¼µÄÉÏÒ»¸ö°üµÄflag Óë·½Ïò*/
	ct->proto.tcp.last_index = index;
	ct->proto.tcp.last_dir = dir;
	...
	/*original·½Ïò¼ì²éµ½ÁËIPS_SEEN_REPLY_BIT¡¢»ØÓ¦·½ÏòÓÐÊý¾Ý*/
	...
	/*Ë«ÏòÁ´½ÓÒÑ½¨Á¢¡¢¾É×´Ì¬ÎªSYN_RECV»òÒÑ½¨Á¢Á´½Ó¡¢ÇÒÐÂ×´Ì¬ÎªÒÑ½¨Á¢Á´½ÓTCP_CONNTRACK_ESTABLISHED*/
...
/*******************************************************************************
 º¯ÊýÃû³Æ :  tcp_new
 ¹¦ÄÜÃèÊö :   tcp±¨ÎÄÏà¹ØÉèÖÃ
 			  1.L4²ã×Ö¶ÎÈçwindow, ackµÈ×Ö¶Î¸³¸øct->proto.tcp.seen[0],ÐÂ½¨Á¢µÄÁ¬½Ó²Åµ÷ÕâÀï£¬ËùÒÔ²»ÓÃ¸øreply·½ÏòµÄct->proto.tcp.seen[1]¸³Öµ 
			  2.ÉèÖÃTCPµÄ×´Ì¬ct->proto.tcp.state=TCP_CONNTRACK_NONE(ÐÂ½¨)
			  3.½«ct->tuplehash¼ÓÈëµ½ÁË net->ct.unconfirmed£¬Î´È·ÈÏÁ´½Ó
 ÊäÈë²ÎÊý :  ct---nf_conn½á¹¹£¬ÐÂ½¨µÄÒ»ÌõÁ´½Ó¸ú×ÙÏî
 			 skb---skb±¨ÎÄ
 			 dataoff---ËÄ²ãÊý¾Ý²¿·ÖÆ«ÒÆ
 			 timeouts---³¬Ê±Ê±¼ä
 Êä³ö²ÎÊý :  ÎÞ
 ·µ»ØÖµ¡¡ :  ÎÞ
 --------------------------------------------------------------------------------
...
static bool tcp_new(struct nf_conn *ct, const struct sk_buff *skb, unsigned int dataoff, unsigned int *timeouts)
{
	...
	/*Á´½Ó¸ú×Ù¶ÔÓ¦µÄnet namespace*/
	struct net *net = nf_ct_net(ct);

	/*namespaceµÄtcpÐÅÏ¢*/
	struct nf_tcp_net *tn = tcp_pernet(net);

	/*nf¼ÇÂ¼µÄ¿Í»§¶Ë·½Ïò tcpµÄ×´Ì¬*/
	const struct ip_ct_tcp_state *sender = &ct->proto.tcp.seen[0];

	/*nf¼ÇÂ¼µÄ ·þÎñ¶Ë·½ÏòtcpµÄ×´Ì¬*/
	...
	/*¸ù¾Ýµ±Ç°±¨ÎÄµÄtcp flag ×ª³öµÄnf tcpÐÂ×´Ì¬*/
	...
	/*µ±Ç°ÎªSYN±¨ÎÄ£¬syn±¨ÎÄÒÑ·¢£¬nfÐÂ×´Ì¬ÎªTCP_CONNTRACK_SYN_SENT*/
	if (new_state == TCP_CONNTRACK_SYN_SENT) 
	{
		/*Çå³ýtcp ×´Ì¬±ê¼Ç*/
		memset(&ct->proto.tcp, 0, sizeof(ct->proto.tcp));

		/* SYN packet£¬origin·½ÏòÌî³ä*/
		/*¼ÇÂ¼µ±Ç°SYN±¨ÎÄÐòÁÐºÅºÍ±¨ÎÄ³¤¶ÈÖ®ºÍ×î´óÖµ*/
		ct->proto.tcp.seen[0].td_end = segment_seq_plus_len(ntohl(th->seq), skb->len, dataoff, th);

		/*¼ÇÂ¼SYN±¨ÎÄ tcp´°¿Ú¿í¶È*/
		ct->proto.tcp.seen[0].td_maxwin = ntohs(th->window);

		/*ÈôSYN±¨ÎÄ´°¿Ú¿í¶ÈÎª0ÉèÎª1*/
		if (ct->proto.tcp.seen[0].td_maxwin == 0)
			ct->proto.tcp.seen[0].td_maxwin = 1;

		/*ack+»¬¶¯´°¿Ú×î´óÖµ = ÐòÁÐºÅ+±¨ÎÄ³¤¶È*/
		/*¼ÇÂ¼SYN±¨ÎÄ ack+»¬¶¯´°¿Ú×î´óÖµ*/
