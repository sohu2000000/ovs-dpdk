...
int tcp_v4_do_rcv(struct sock *sk, struct sk_buff *skb)
{
	...
	// 若是已连接套接字
	//如果状态是TCP_ESTABLISHED，表明连接已经建立，
	//调用tcp_rcv_established函数将SKB加入sk-> receive_queue中
	//tcp_rcv_established中语句 __skb_queue_tail(&sk->sk_receive_queue, skb);
	//完成队列的添加
	...
	// 若是监听套接字
	//如果sk的状态为TCP_LISTEN，则调用tcp_v4_hnd_req函数生成
    //一个新的sock用于传输
	if (sk->sk_state == TCP_LISTEN) {
...
int tcp_v4_rcv(struct sk_buff *skb)
{
	...
	/* 取tcp头 */
	th = (const struct tcphdr *)skb->data;
	// ip_hdr() -> skb_network_header() -> skb->network_header
	/* 取ip头 */
	iph = ip_hdr(skb);
lookup:
