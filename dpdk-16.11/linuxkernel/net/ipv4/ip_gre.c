...
static int __init ipgre_init(void)
{
	...
	/*gre网络操作函数对象注册到pernet_list网络链*/
	err = register_pernet_device(&ipgre_net_ops);
	if (err < 0)
		return err;

	/*ipgre_tap_net_ops注册到pernet_list链*/
	err = register_pernet_device(&ipgre_tap_net_ops);
	if (err < 0)
		goto pnet_tap_failed;

	/*erspan_net_ops注册到pernet_list链*/
	...
	/*ipgre_link_ops添加到全局netlink操作函数链表*/
	err = rtnl_link_register(&ipgre_link_ops);
	if (err < 0)
		goto rtnl_link_failed;

	/*ipgre_tap_ops添加到全局netlink操作函数链表*/
	err = rtnl_link_register(&ipgre_tap_ops);
	if (err < 0)
		goto tap_ops_failed;

	/*erspan_link_ops添加到全局netlink操作函数链表*/
