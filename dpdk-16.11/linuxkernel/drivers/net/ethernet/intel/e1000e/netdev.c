...
/*******************************************************************************
 函数名称 :  e1000_intr
 功能描述 :  中断处理程序
 			 将数据包放在buffer_info的缓冲块中()，
 			 就是讲总线地址指向的内容复制到skb中，
 			 然后根据skb中的协议将其传给上层协议的接收函数

 			 这样当第一次接收到rx的中断时，在e1000_intr 
 			 中通过napi_schedule_prep 来判断napi是否可以被调动
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
static irqreturn_t e1000_intr(int __always_unused irq, void *data)
{
	...
	/*确定该设备处于运行状态, 而且还未被添加到网络层的poll队列中,
	  这样当第一次接收到rx的中断时，在e1000_intr 中通过napi_schedule_prep 
	  来判断napi是否可以被调动*/
	if (napi_schedule_prep(&adapter->napi)) {
		adapter->total_tx_bytes = 0;
		adapter->total_tx_packets = 0;
				...
				struct e1000_ring *ring)
{
	...
	/*一致性dma内存申请*/
	ring->desc = dma_zalloc_coherent(&pdev->dev, ring->size, &ring->dma, GFP_KERNEL);
	if (!ring->desc)
		return -ENOMEM;
...
/*******************************************************************************
 函数名称  :	e1000e_setup_rx_resources
 功能描述  :	建立接收发送缓冲区
 输入参数  :  
 输出参数  :	
 返 回 值  :	无
...
static void e1000_configure(struct e1000_adapter *adapter)
{
	...
	/*从这里就可以看出 环形缓冲区并不是一开始就完全建好的,建了部分*/
...
int e1000e_open(struct net_device *netdev)
{
	...
	/*当用户敲ifconfig up命令时，最终调用网卡驱动的open函数*/
 ...
 **/
 ...
 /*******************************************************************************
 函数名称 :  e1000e_close
 功能描述 :  关闭中断，然后释放中断号，并且会释放网卡申请的相应的空间
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称 :  e1000_tx_queue
 功能描述 :  根据相应的参数找到缓冲块存放，
 			 缓冲块中有dma成员，表示该数据包所在的总线地址，
 			 控制总线会把内容映射到总线地址，然后由网卡传送出去
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称 :  e1000_netpoll
 功能描述 :  中断驱动程序都采用的是NAPI方式，
 			 需要提供poll函数，本驱动中是e1000_netpoll轮询函数

 			 新数据包要发送时候，首先上层协议调用e1000_xmit_frame函数，
 			 然后在该函数中调用e1000_tx_queue来根据相应的参数找到缓冲块存放，
 			 缓冲块中有dma成员，表示该数据包所在的总线地址，控制总线会把内容映射到总线地址，
 			 然后由网卡传送出去

 			 当有新数据包达到时，首先触动中断处理函数e1000_intr，
 			 该中断函数会将数据包放在buffer_info的缓冲块中()，
 			 就是讲总线地址指向的内容复制到skb中，然后根据skb中的协议将其传给上层协议的接收函数。
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
static const struct net_device_ops e1000e_netdev_ops = {
	...
	.ndo_start_xmit		= e1000_xmit_frame,                     /*协议层向下发数据包接口*/
...
#ifdef CONFIG_NET_POLL_CONTROLLER
	.ndo_poll_controller	= e1000_netpoll,                    /*轮询函数*/
#endif
 ...
 **/
 /*******************************************************************************
 函数名称 :  e1000_probe
 功能描述 :  完成设备初始化工作,包括pci_dev结构体，
 			 操作系统初始化，e1000_adapter结构体配置，和硬件重启工作
 			 实现dma映射
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
