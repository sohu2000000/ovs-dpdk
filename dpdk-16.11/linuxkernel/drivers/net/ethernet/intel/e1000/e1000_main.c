...
static const struct pci_error_handlers e1000_err_handler = {
	.error_detected = e1000_io_error_detected,                    //错误检查
	.slot_reset = e1000_io_slot_reset,                            //槽位重设
	.resume = e1000_io_resume,                                    //io重启
};
...
e1000_init_module(void)
{
	...
	/*驱动注册，检查驱动是否加载到总线，没有则加载，加载完调用probe*/
...
static int e1000_request_irq(struct e1000_adapter *adapter)
{
	...
	第三个参数flags中断处理程序的标志，这个标志可以是一个也可以是多个，
	列举几个最重要的标志：
	IRQF_DISABLED: 
	该标志被设置后，意味内核在处理中断处理程序本身的时候，
	禁止了其他所有的中断。如果不设置，中断处理程序可?以与除本身之外的其他任何中断同时运行。
	显而易见我们一般不去这么野蛮的设置这个标志
	IRQF_TIMER:
	为系统定时器的中断处理而准备的
	IRQF_SHARED:
	这个中断标志经常能遇见，这个标志意思就是多个中断处理程序之间可以共享中断线，
	概括起来就是没有这个标志就只能独自一个人占用，标志了，就是很多人可以占用这个中断号来
	
	第四个才参数就是自定义与中断设备相关的文本了
	第五个参数dev，看到第三个参数中IRQF_SHARED时候，你会不会有这样的疑问，
	假如现在我要释放当前共享的指定这个中断程序时候，我如何释放？会不会把其他占用也会删除掉，
	这就是第五个参数的意义，如果中断线是共享的，那么就必须传递能够代表当前设备的唯一信息
	request_irq()成功返回0，如果返回非0，就表示有错误发生，这个时候你可以考虑当前中断是否被占用了，
	所以可以加上IRQF_SHARED标志
	*/
...
/*e1000，Intel千兆网卡网卡操作函数*/
static const struct net_device_ops e1000_netdev_ops = {
	.ndo_open			= e1000_open,                     /*网卡激活时调用*/
	.ndo_stop			= e1000_close,
	.ndo_start_xmit		= e1000_xmit_frame,               /*发函数*/
...
static int e1000_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	adapter->bars = bars;                                                           /*pci bar 寄存器存储地址总线地址*/
	...
	/*映射pci bar0 记录的pci基地址到内核地址空间，返回虚拟地址*/
	...
	/*适配器的napi_struct结构挂入设备的napi_struct链表，中断来临时由中断处理函数遍历调用*/
...
/*******************************************************************************
 函数名称 :  e1000_open
 功能描述 :  打开设备使用的函数为e1000_open。当网络接口激活的时候被调用
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
int e1000_open(struct net_device *netdev)
{
	struct e1000_adapter *adapter = netdev_priv(netdev);  /*网络适配器结构*/
	struct e1000_hw *hw = &adapter->hw;                   /*硬件相关*/
	int err;
 ...
 **/
 /*******************************************************************************
 函数名称 :  e1000_setup_all_tx_resources
 功能描述 :  申请发送队列资源
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
				    ...
				    struct e1000_rx_ring *rxdr)
{
	...
	/*真实报文mbuf内存区*/
	/*rx bd分配一致性dma内存，返回内存的虚拟地址，物理地址保存在rxdr->dma,4k dma内存*/
...
static void e1000_configure_rx(struct e1000_adapter *adapter)
{
	...
	if (adapter->netdev->mtu > ETH_DATA_LEN) {
		...
		rdlen = adapter->rx_ring[0].count *
			sizeof(struct e1000_rx_desc);
		adapter->clean_rx = e1000_clean_rx_irq;                    /*从队列中取报文*/
		adapter->alloc_rx_buf = e1000_alloc_rx_buffers;            /*申请报文缓存*/
	}
...
/*******************************************************************************
 函数名称 :  e1000_free_all_tx_resources
 功能描述 :  释放发送资源
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 ...
 **/
 ...
 函数名称 :  e1000_request_irq
 功能描述 :  e1000网卡中断处理函数
 			 当第一次接收到rx的中断时，在e1000_intr 中通过napi_schedule_prep 来判断napi是否可以被调动
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
static irqreturn_t e1000_intr(int irq, void *data)
{
	...
	/*判断napi是否可以被调动*/
	if (likely(napi_schedule_prep(&adapter->napi))) {	//设置state为NAPI_STATE_SCHED
		adapter->total_tx_bytes = 0;
		adapter->total_tx_packets = 0;
		adapter->total_rx_bytes = 0;
		adapter->total_rx_packets = 0;

		/*可以调动的话调用schedule 触发软中断*/ //将设备添加到 poll list，并开启软中断
...
/*******************************************************************************
 函数名称 :  e1000_alloc_rx_skb
 功能描述 :  动态申请skb内存 kmem_cache_alloc模式
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称 :  e1000_clean_jumbo_rx_irq
 功能描述 :  napi_struct结构填到dev的napi_struct链
 输入参数 :  接收中断号
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称 :  e1000_copybreak
 功能描述 :  报文从环队列buffer_info->data拷贝到skb内存
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称 :  e1000_clean_rx_irq
 功能描述 :  设备轮询接收机制中最重要的函数,收完包清除接收中断
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
			       ...
			       int *work_done, int work_to_do)
{
	...
	/*ext_to_clean是下一个可以被清除的描述符索引,
	上面讲过环形缓冲队列由多个描述符组成,
	每个描述符都有一个用于存放接收数据包的缓冲区buffer,
	这里所说的“可以被清除”并不是将其删除,
	而是标记这个缓冲区的数据已经处理(可能正在处理),
	但是否处理完了要看rx_desc->status&E1000_RXD_STAT_DD,
	当有新数据需要使用缓冲区时,只是将已处理的缓冲区覆盖而已,
	这里的i可以理解为可以被新数据覆盖的缓冲区序号
*/
	i = rx_ring->next_to_clean;
	...
	while (rx_desc->status & E1000_RXD_STAT_DD) {
		...
		/*报文从环队列buffer_info->data拷贝到skb内存，memcpy, 动态kmem_cache_alloc申请的skb内存*/
		...
		if (!skb) {
			...
			/*取消映射，因为通过DMA,网卡已经把数据放在了主内存中,
			这里一取消,也就意味着,CPU可以处理主内存中的数据了 */
			dma_unmap_single(&pdev->dev, buffer_info->dma,
					 adapter->rx_buffer_len,
					 DMA_FROM_DEVICE);
...
next_desc:
		...
		if (unlikely(cleaned_count >= E1000_RX_BUFFER_WRITE)) {
			...
			/*在e1000_up中已经调用了这个函数为环形缓冲区
			  队列中的每个缓冲区分配了sk_buff内存，但是如果接收到数据以后，
			  调用netif_receive_skb(skb)向上提交数据以后，
			  这段内存将始终被这个skb占用(直到上层处理完以后才会调用_kfree_skb释放),
			  换句话说,就是当前缓冲区必须重新申请分配sk_buff内存,为下一个数据作准备
			*/
