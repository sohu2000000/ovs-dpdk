...
struct ixgbe_adapter
{
	...
	struct vlan_group *vlgrp; /* must be first, see ixgbe_receive_skb */    /*vlan组*/
#else
	unsigned long active_vlans[BITS_TO_LONGS(VLAN_N_VID)];                  /*活动的VLAN*/
#endif
...
#define IXGBE_FLAG2_OVERFLOW_CHECK_ENABLED      (u32)(1 << 12)
	...
	/* Tx fast path data */
	int num_tx_queues;                                                      /*发送队列数*/
	u16 tx_itr_setting;
	u16 tx_work_limit;

	/* Rx fast path data */
	int num_rx_queues;                                                      /*接收队列数*/
	u16 rx_itr_setting;
	u16 rx_work_limit;
