...
struct ofproto {
    struct hmap_node hmap_node; /* In global 'all_ofprotos' hmap. */							/*链表头*/
    //const struct ofproto_class *ofproto_class;												/*openflow交换机实现类，用来实现一个openflow交换机。主要包括创建、构造、操作openflowflow等相关方法*/
    const struct ofproto_class *ofproto_class_zwl;												/*openflow交换机实现类，用来实现一个openflow交换机。主要包括创建、构造、操作openflowflow等相关方法*/
    char *type;                 /* Datapath type. */											/*网桥类型 netdev*/
    char *name;                 /* Datapath name. */											/*dp name*/

    /* Settings. */
    uint64_t fallback_dpid;     /* Datapath ID if no better choice found. */					/*备选dp id*/
    uint64_t datapath_id;       /* Datapath ID. */												/*dp id*/
    bool forward_bpdu;          /* Option to allow forwarding of BPDU frames
                                 * when NORMAL action is invoked. */
    char *mfr_desc;             /* Manufacturer (NULL for default). */
    char *hw_desc;              /* Hardware (NULL for default). */								/*hw 描述符*/
    char *sw_desc;              /* Software version (NULL for default). */						/*软件描述符*/
    char *serial_desc;          /* Serial number (NULL for default). */
    char *dp_desc;              /* Datapath description (NULL for default). */
    enum ofputil_frag_handling frag_handling;													/*分片处理*/

    /* Datapath. */
    struct hmap ports;          /* Contains "struct ofport"s. */								/*端口链表*/
    struct shash port_by_name;																	/*端口名链表*/
    struct simap ofp_requests;  /* OpenFlow port number requests. */							/*端口号*/
    uint16_t alloc_port_no;     /* Last allocated OpenFlow port number. */						/*最后分配的端口号*/
    uint16_t max_ports;         /* Max possible OpenFlow port num, plus one. */					/*端口号最大值*/
    struct hmap ofport_usage;   /* Map ofport to last used time. */								/**/
    uint64_t change_seq;        /* Change sequence for netdev status. */						/*netdev 序号*/

    /* Flow tables. */
    long long int eviction_group_timer; /* For rate limited reheapification. */
    struct oftable *tables;																		/*of层table*/
    int n_tables;
    ...
    struct hmap meters;             /* uint32_t indexed 'struct meter *'.  */			/*of层meter hmap*/
    ...
    struct connmgr *connmgr;															/*openflow 链接管理结构，一个of 一个*/
...
struct ofport {
    ...
    struct netdev *netdev;															/*端口对应的netdev设备*/
    struct ofputil_phy_port pp;													/*端口对应物理port*/
    ofp_port_t ofp_port;        /* OpenFlow port number. */							/*of端口号*/
    uint64_t change_seq;															/*序列号*/
...
/* ## --------------------------- ## */
    ...
    int (*construct)(struct ofproto *ofproto);										/*construct*/
