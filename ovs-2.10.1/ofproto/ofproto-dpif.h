...
/*数据面backer，管理数据面dp的资源，相同类型的dp共享一个backer*/
/* All datapaths of a given type share a single dpif backer instance. */
struct dpif_backer {
    char *type;																				/*数据面类型*/
    int refcount;																			/*backer应用次数*/
    //struct dpif *dpif;
	struct dpif *dpif_zwl;																	/*dp对应的dpif结构*/
    struct udpif *udpif_zwl;																/*数据面接口*/
    //struct udpif *udpif;																	/*upcall dpif结构*/

    struct ovs_rwlock odp_to_ofport_lock;
    struct hmap odp_to_ofport_map OVS_GUARDED; /* Contains "struct ofport"s. */

    struct simap tnl_backers;      /* Set of dpif ports backing tunnels. */					/*dp端口*/

    enum revalidate_reason need_revalidate; /* Revalidate all flows. */						/*老化flow*/

    bool recv_set_enable; /* Enables or disables receiving packets. */						/*dp是否使能收包*/

    /* Meter. */
    struct id_pool *meter_ids;     /* Datapath meter allocation. */							/*dp meter id池子*/

    /* Version string of the datapath stored in OVSDB. */
    char *dp_version_string;																/*dp的版本*/

    /* Datapath feature support. */
    struct dpif_backer_support bt_support;   /* Boot time support. Set once					/*dp 特征支持*/
...
/*openflow 交换机的接口层，ofproto 接口类的具体实现*/
...
struct ofproto_dpif {
    ...
    struct ofproto up;															    /*openflow交换机*/																
    struct dpif_backer *backer;												    /*dpif关联的backer*/
