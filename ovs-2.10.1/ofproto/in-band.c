...
/*******************************************************************************
 ...
 功能描述  :    因为OpenvSwitch不仅仅是一个OpenFlow Switch，它的流表组成除了of流表外，还有其他一些（隐藏）流表。
 				这些隐藏流表是由于默认交换机和控制器在同一网络中（in-band），因此要保证两者互通。
 				要关闭默认的inband可以通过“ovs-vsctl set controller br0 connection-mode=out-of-band”
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
