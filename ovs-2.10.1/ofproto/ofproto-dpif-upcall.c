...
struct handler {
    ...
    uint32_t handler_id;               /* Handler id. */							/*handler线程id*/
};
 ...
 */
struct revalidator {
    struct udpif *udpif;               /* Parent udpif. */					/*upcall结构*/
    pthread_t thread;                  /* Thread ID. */						/*线程id*/
    unsigned int id;                   /* ovsthread_id_self(). */			/**/
};


/*upcall 与dp交互的接口结构*/
/*upcall handler ofproto_dpif*/
...
struct udpif {
    ...
    struct handler *handlers;          /* Upcall handlers. */								/*handler结构指针 不止一个*/
    size_t n_handlers;																		/*handler线程数*/

    struct revalidator *revalidators;  /* Flow revalidators. */								/*revalidator指针数组，每线程1个*/
    ...
    bool reval_exit;                   /* Set by leader on 'exit_latch. */					/*设置停止线程*/
    struct ovs_barrier reval_barrier;  /* Barrier used by revalidators. */
    struct dpif_flow_dump *dump;       /* DPIF flow dump state. */							/*dump结构，描述一次dump流表的情况*/
    long long int dump_duration;       /* Duration of the last flow dump. */				/*整个dump的耗时*/
    struct seq *dump_seq;              /* Increments each dump iteration. */				/*全局dump序号，多线程*/
     ...
     * revalidation round. */
    bool pause;                        /* Set by leader on 'pause_latch. */					/*暂停线程*/
    ...
    atomic_uint n_flows;               /* Number of flows in the datapath. */				/*dp 的flow总量*/
    atomic_llong n_flows_timestamp;    /* Last time n_flows was updated. */					/*flow最后一次更新时间*/
...
struct ukey_op {
    ...
    struct dpif_flow_stats stats; /* Stats for 'op'. */		/*统计*/
    struct dpif_op dop;           /* Flow operation. */		/*要对flow执行的操作*/
};
...
/*******************************************************************************
 函数名称  :    udpif_stop_threads
 功能描述  :    停掉handler线程和revalidator线程
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    udpif_start_threads
 功能描述  :    启动handler线程和revalidator线程
 输入参数  :  	udpif---upcall dpif
		 		n_handlers_---handler线程个数
		 		n_revalidators_---revalidator线程数
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
/*******************************************************************************
 函数名称  :	udpif_set_threads
 功能描述  :	多线程
 输入参数  :    n_handlers_---配置的线程个数
                n_revalidators_---配置的revalidator线程个数
 输出参数  :	
 返 回 值  :	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
                  ...
                  size_t n_revalidators_)
{
    ...
    if (!udpif->handlers && !udpif->revalidators) {
		...
		/*upcall dp 接口 线程启动启动handler和upcall线程*/
 ...
 * in dpif. */
/*******************************************************************************
 函数名称  :    udpif_upcall_handler
 功能描述  :    用户态接收内核发送的upcall信息，有线程监听消息，一旦有消息，则触发udpif_upcall_handler
 				udpif_upcall_handler会在udpif_start_threads里面初始化
 				同时创建的还有udpif_revalidator的线程
...
/*******************************************************************************
 函数名称  :    ovs_vport_send
 功能描述  :    读取upcall，一次处理UPCALL_MAX_BATCH个请求
 输入参数  :    vport---出端口
 				skb---要出的包
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    udpif_revalidator
 功能描述  :    ovs_thread_create 创建的revalidator线程，dpcls flow 没有报文命中老化删除
 输入参数  :  	arg---struct revalidator 指针数组
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
udpif_revalidator(void *arg)
{
	...
	for (;;) {
        ...
        if (leader) {
			...
			//revalidator需求序号，need_revalidate 设置true后序号加1，唤醒revalidator线程
			...
			if (!udpif->reval_exit) {
				...
				/*创建dump flow的结构 描述一次dump流表，只需第一个revalidator线程创建一次，所有revalidator线程共用*/
        ...
        if (leader) {
			...
			/*为啥修改flow_limit，爬一遍太慢，要老化快一点*/
			if (duration > 2000) {
                /*dp上flow太多了，需要调小限制的flow数, 老化快些*/
...
/*******************************************************************************
 函数名称  :    process_upcall
 功能描述  :    ovs出接口，从虚拟端口处，根据upcall的类型MISS_UPCALL确定调用函数upcall_xlate
 输入参数  :    vport---出端口
 				skb---要出的包
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    handle_upcalls
 功能描述  :    将flow rule添加到内核中的datapath
 输入参数  :    vport---出端口
 				skb---要出的包
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    ukey_create_from_dpif_flow
 功能描述  :    根据flow创建ukey
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
    ...
    OVS_TRY_LOCK(0, (*result)->mutex)
{
    ...
    if (ukey) {
		...
		/*通常不会从revalidator 插入dp flow 但ovs-vswitchd重启时会从revalidator插入flow*/
...
/*******************************************************************************
 函数名称  :    revalidate_ukey
 功能描述  :    获取revalidate upcall的key
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    revalidate
 功能描述  :    线程老化主流程，每次dump pmd上50条flow，超时删除
 输入参数  :  	revalidator---dump流表结构
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
revalidate(struct revalidator *revalidator)
{
	...
	/*读取udpif的全局dump序号，线程执行完一次dump会加1，并发*/
    dump_seq = seq_read(udpif->dump_seq);

	/*reval的序号*/
	reval_seq = seq_read(udpif->reval_seq);

	/*获取限制的flow总数，默认限制20w flow，动态调整*/
    atomic_read_relaxed(&udpif->flow_limit, &flow_limit);

	/*创建struct dpif_flow_dump_thread结构，每个revalidator线程都会创建自己的struct dpif_flow_dump_thread结构*/
    ...
    for (;;) {
		...
		//从datapath dump flow信息，返回dump到的flow个数，每次最多50条flow，返回dump到的flow个数
		...
		//如果当前flow个数超过flow_limit的两倍，则删除获取的所有flow，默认limit 20w flow
		...
		/*当前flow数超flow_limit 设置老化时间为100ms，否则是默认10s*/
		max_idle = n_dp_flows > flow_limit ? 100 : ofproto_max_idle;

		/*每次处理50个flow，如果超时没命中打上删除标记执行删除op*/
        for (f = flows; f < &flows[n_dumped]; f++) {
			...
			/*对flow 提取key 提取不到continue*/
            if (ukey_acquire(udpif, f, &ukey, &error)) {
				/*另一个线程在处理本flow，ukey被lock了*/
				if (error == EBUSY) {
			...
			/*最新命中flow时间为0，flow是revalidator新创建的*/
            if (!used) {
                used = ukey->created;
            }
		...
		//和datapath交互，删除或者修改flow，openflow 变更则是修改，如果是超时老化则是删除
		/*与flow数相等，修改或删除*/
        if (n_ops) {
...
/*******************************************************************************
 函数名称  :    upcall_unixctl_set_flow_limit
 功能描述  :    设置upcall流表数限制
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
