...
/*全局dpif backer链，所有dp类型的backer挂链*/
...
/*******************************************************************************
 函数名称  :    type_run
 功能描述  :    启动某类型的dp的ofproto, struct ofproto_class type_run
 				1.启动对应backer类型的交换机，同类型交换机只有一个数据面，dpdk类型netdev的backer最终启动pmd线程开始poll收包 
 				dp_netdev 创建网桥时生成，同类型只能有一个
 输入参数  :  	type---netdev
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
{
	/*相同类型数据面共享一个backer，管理dp资源，dpdk类型只有一个netdev类型的backer，同类型所有网桥共享一个backer*/
    struct dpif_backer *backer;

    /*根据datapath type查找backer，即相同的datapath type只有一个backer，dpdk是netdev类型的backer*/
	...
	/*backer已存在，启动对应网桥dp if层 交换机，最终启动pmd线程开始poll收包*/
	...
	/*开启了收包功能，启动handler和revalidator线程*/
...
/*******************************************************************************
 函数名称  :    open_dpif_backer
 功能描述  :    打开或创建backer ovs-netdev
 输入参数  :  	type---dp的类型，ovs-netdev
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
open_dpif_backer(const char *type, struct dpif_backer **backerp)
{
	...
	/*根据类型netdev获取本类型下所有dp name*/
    dp_enumerate_names(type, &names);

	/*查找backername链表，是否已存在 ovs-netdev类型的backer*/
	SSET_FOR_EACH(name, &names) {
        struct dpif *old_dpif;

		/*backer name 对应的dp已存在*/
	...
	/*创建或打开dp类型对应的backer，dp_netdev的创建*/
...
/*******************************************************************************
 函数名称  :    construct
 功能描述  :    构建of层交换机, 注册在struct ofproto_class
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
construct(struct ofproto *ofproto_)
{
	...
	/*创建ofproto backer，ofproto_dpif入口，一种类型的桥只有一个backer，type 为netdev*/
...
/*******************************************************************************
 函数名称  :    rule_dpif_lookup_from_table
 功能描述  :    查找指定table的流表，默认是table 0，用一个循环去遍历每一个table，然后知道找到匹配的rule
 				如果报文分片，默认是设置源目的端口都设置为0，其他情况下丢弃报文
 				遍历所有的table，每次都会调用rule_dpif_lookup_in_table去查找rule，
 				如果找到最终找到之后返回，找不到的话就会去下一个table找
          ...
          struct ofputil_meter_config *config)
{
	...
	/*meter id 校验，meterid 未知，在dp分配一个meter id*/
...
#if 1
	VLOG_DBG("zwl-of-delmeterflow free_meter_id meter_id=%u.", args->meter_id.uint32);
#endif
