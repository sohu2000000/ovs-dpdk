...
/* Configuration that persists from one connection to the next. */
    ...
    struct rconn *rconn;        /* OpenFlow connection. */									/*openflow 链接*/
...
static struct ofservice *ofservice_lookup(struct connmgr *,
                                          const char *target);
/*of 链接关联，关联of 请求链接，与控制器链接*/
/* Connection manager for an OpenFlow switch. */
struct connmgr {
    struct ofproto *ofproto;	/*of层交换机*/
    char *name;
    char *local_port_name;

    /* OpenFlow connections. */
    struct hmap controllers;     /* All OFCONN_PRIMARY controllers. */
    struct ovs_list all_conns;   /* All controllers.  All modifications are			//向控制器的主动链接
...
/*******************************************************************************
 函数名称  :    connmgr_run
 功能描述  :    ovs-vswitchd的网络连接管理,设置当有openflow调用的时候，handle_openflow会被调用
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
	...
	/*遍历所有of层链接 调用handle_openflow 处理报文，链接控制器相关链接*/
    LIST_FOR_EACH_SAFE (ofconn, next_ofconn, node, &mgr->all_conns) {
    	/*主动链接的处理流程*/
	...
	/*处理作为服务端的链接 of请求等，处理of的vconn链接请求*/
    HMAP_FOR_EACH (ofservice, node, &mgr->services) {
        struct vconn *vconn;
        int retval;

		/*最终将调用pstream_class->accept实现与punix，pssl，ptcp等被动连接，服务端*/
		/*ofctl 走Unix socket 进程间通信，建立唯一链接*/
...
/*******************************************************************************
 函数名称  :    ofconn_run
 功能描述  :    对connmgr所有的主动连接all_conns产生的ofconn进行处理
 				主要完成网桥与控制器连接的状态转移管理和of消息的接收处理等
 				控制器交互的主动链接处理
 输入参数  :  	ofconn--of层链接
 输出参数  :	
 返 回 值  : 	无
                                   ...
                                   const struct ofpbuf *ofp_msg))
{
    ...
    for (i = 0; i < 50 && ofconn_may_recv(ofconn); i++) {
		...
		//调用handle_openflow对openflow消息具体处理
