...
static const struct ofproto_class **ofproto_classes;	/*of层类全局指针数组*/
static size_t n_ofproto_classes;						/*of层类个数，bridge个数*/
static size_t allocated_ofproto_classes;				/*申请的of类*/
...
/*upcall hundler线程和revalidator线程的个数*/
...
ofproto_init(const struct shash *iface_hints)
{
	...
	/*注册of层交换机操作类ofproto_dpif_class，ofproto的执行函数库集合*/
	/*of层class注册ofproto_dpif_class 赋值给ofproto_classes，注册了操作函数库才可以创建ofproto，
	  在bridg_reconfigure()中调用ofproto_create创建ofproto*/
    ...
    SHASH_FOR_EACH(node, iface_hints) 
    {
		...
		/*拷贝一份端口放入ofp_ports，添加到of层全局端口链表init_ofp_ports*/
    ...
    for (i = 0; i < n_ofproto_classes; i++) {
		...
		/*端口初始化ofproto_dpif_class->init() init 用户态初始化*/
...
/*******************************************************************************
 函数名称  :    ofproto_class_register
 功能描述  :    of层class注册ofproto_dpif_class 赋值给ofproto_classes
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    ofproto_create
 功能描述  :    of层交换机创建，注册ofproto_dpif_class之后，才可以创建ofproto
 输入参数  :  	datapath_name---br-vrouter
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
/*******************************************************************************
 函数名称  :    ofproto_reconnect_controllers
 功能描述  :    断开强制重连
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    ofproto_type_run
 功能描述  :    启动ofproto，datapath开始收包，netdev类型 dpdk收包
 				1.启动某类型的网桥的数据面开始收包，同类型的网桥只有一个backer
 输入参数  :  	datapath_type---netdev型
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
ofproto_type_run(const char *datapath_type)
{
	...
	/*获取网桥的type netdev 是dpdk类型的datapath*/
    datapath_type = ofproto_normalize_type(datapath_type);

	/*根据网桥类型查找对应的类，netdev类型的类*/
	class = ofproto_class_find__(datapath_type);
...
ofproto_run(struct ofproto *p)
{
	...
	/*of层交换机启动 run ，p为protol实例 ofproto 函数run*/
    ...
    if (p->eviction_group_timer < time_msec()) {
        ...
        for (i = 0; i < p->n_tables; i++)
		{
			...
			{
                static struct vlog_rate_limit count_rl = VLOG_RATE_LIMIT_INIT(1, 1);
                VLOG_WARN_RL(&count_rl, "Table %"PRIuSIZE" has an excessive" " number of rules: %d", i, table->n_flows);
            }
...
/*******************************************************************************
 函数名称  :    handle_flow_mod
 功能描述  :    首先将openflow协议解析为fm和ofpacts
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    ofproto_add_meter
 功能描述  :    meter 节点插入of proto 链表
 输入参数  :  	meter---新生成的meter node
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
/*******************************************************************************
 函数名称  :    emc_cache_slow_sweep
 功能描述  :    emc流表删除
 输入参数  :  	config---解析出的meter配置
 				provider_meter_id---meter资源id
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
#if 1
	VLOG_DBG("zwl-of-delmeterflow meter_delete meter_id=%d.", meter_id);
#endif
...
/*******************************************************************************
 函数名称  :    handle_add_meter
 功能描述  :    of层申请meter资源插入hmap
 输入参数  :  	mm---meter配置
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
handle_add_meter(struct ofproto *ofproto, struct ofputil_meter_mod *mm)
{
...
#if 1
	/*加debug信息记录 要下发的meter id*/
	VLOG_DBG("zwl-of-handle_add_meter  mm->meter.meter_id=%d.",  mm->meter.meter_id);
#endif
	...
	/*设置meter of 层设置meter meter_set  后面dp层申请meter资源 关联到dp*/
...
/*******************************************************************************
 函数名称  :    handle_modify_meter
 功能描述  :    meter mod 调用set接口
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    handle_meter_mod
 功能描述  :    open flow meter操作入口
 输入参数  :  	ofconn---of层链接
 			    oh---openflow消息数据头
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    handle_openflow__
 功能描述  :    处理openflow
 输入参数  :  	ofconn---open flow 链接
 				msg---open flow 消息
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
