...
struct iface {
	...
	//struct netdev *netdev;      /* Network device. */							/*端口对应netdev*/
    struct netdev *netdev_zwl;      /* Network device. */							/*端口对应netdev*/
    ofp_port_t ofp_port;        /* OpenFlow port number. */
    uint64_t change_seq;

    /* These members are valid only within bridge_reconfigure(). */
    const char *type;           /* Usually same as cfg->type. */
    const struct ovsrec_interface *cfg;											/*接口对应数据库数据各式*/
};
...
struct port {
    ...
    struct ovs_list ifaces;    /* List of "struct iface"s. */						/*接口链*/
};

/*网桥结构面对用户*/
struct bridge {
    struct hmap_node node;      /* In 'all_bridges'. */								/*连接所有网桥*/
    char *name;                 /* User-specified arbitrary name. */				/*网桥name*/
    char *type;                 /* Datapath type. */								/*数据面类型*/
    struct eth_addr ea;         /* Bridge Ethernet Address. */						/*网桥mac*/
    struct eth_addr default_ea; /* Default MAC. */									/*网桥默认mac*/
    const struct ovsrec_bridge *cfg;

    /* OpenFlow switch processing. */
    //struct ofproto *ofproto;    /* OpenFlow switch. */							/*连接of层openflow交换机*/
    struct ofproto *ofproto-zwl;    /* OpenFlow switch. */							/*连接of层openflow交换机*/

    /* Bridge ports. */
    struct hmap ports;          /* "struct port"s indexed by name. */				/*网桥上端口链*/
    struct hmap ifaces;         /* "struct iface"s indexed by ofp_port. */			/*接口链表*/
    struct hmap iface_by_name;  /* "struct iface"s indexed by name. */				/*端口name链表*/

    /* Port mirroring. */
    struct hmap mirrors;        /* "struct mirror" indexed by UUID. */				/*镜像端口*/

    /* Auto Attach */
    struct hmap mappings;       /* "struct" indexed by UUID */						/**/

    /* Used during reconfiguration. */
    struct shash wanted_ports;														/*希望的端口*/
...
struct netdev_stats zwl_stats_pre[2];


/*******************************************************************************
 函数名称  :    bridge_init_ofproto
 功能描述  :    注册ofproto_dpif_class类，对ofproto交换机的操作函数
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    交换机初始化流程
 功能描述  :    连接数据库ovsdb, 并从中读取配置信息
 				主要是初始化网桥bridge模型，通过remote地址，来从OVSDB服务端获取配置信息来实现网桥初始化配置
 				IDL，即接口定义语言，一般用于RPC（远程过程调用）。这里可以理解为用于vswitch和OVSDB服务之间的交互调用，一些信息就需要不断从OVSDB获取，如网桥信息
 输入参数  :  
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    bridge_reconfigure
 功能描述  :    ovs_cfg是从ovsdb-server里面读取出来的配置
 				对于每一个网桥，将网卡添加进去
 输入参数  :    vport---出端口
 				skb---要出的包
 输出参数  :	
...
bridge_reconfigure(const struct ovsrec_open_vswitch *ovs_cfg)
{
	...
	/*遍历网桥,根据数据库更新信息为每个bridge收集新port或者删除port*/
    ...
    HMAP_FOR_EACH_SAFE (br, next, node, &all_bridges) {
		...
		/*网桥没有对应of层网桥则创建ofproto*/
        if (!br->ofproto) {
            int error;

			/* 如果是新加的brdige会分配一个ofproto,实际分配oproto_dpif，创建br对应的backer*/
            error = ofproto_create(br->name, br->type, &br->ofproto);
            if (error) {
                VLOG_ERR("failed to create bridge %s: %s", br->name, ovs_strerror(error));
...
/*******************************************************************************
 函数名称  :    bridge_delete_ofprotos
 功能描述  :    删除of层网桥，结构体ofproto代表一个ovs网桥，其含有of表、meter表、port、连接管理connmgr等属性
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    bridge_delete_or_reconfigure_ports
 功能描述  :    端口删除
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    bridge_add_ports__
 功能描述  :    网桥添加时创建接口
 输入参数  :  	wanted_ports---网桥上端口
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
...
/*******************************************************************************
 函数名称  :    bridge_configure_datapath_id
 功能描述  :    br 配置dp id
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
add_del_bridges(const struct ovsrec_open_vswitch *cfg)
{
    ...
    for (i = 0; i < cfg->n_bridges; i++) 
	{
        ...
        if (strchr(br_cfg->name, '/') || strchr(br_cfg->name, '\\')) {
             ...
             * loss elsewhere. */
            VLOG_WARN_RL(&rl, "ignoring bridge with invalid name \"%s\"",br_cfg->name);
        } 
...
/*******************************************************************************
 函数名称  :    iface_refresh_netdev_status
 功能描述  :    iface刷新接口netdev的状态
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    iface_refresh_stats
 功能描述  :    从dpdk获取一次当前流量，写入数据库
 输入参数  :  	iface--要刷新流量的接口
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
#undef IFACE_STAT
	...
	VLOG_DBG("zwl insert iface name %s statistics in db, stats.rx_bytes=%lu, stats.rx_packets=%u, stats.tx_bytes=%lu, stats.tx_packets=%u", 
		iface->name, stats.rx_bytes, stats.rx_packets, stats.tx_bytes, stats.tx_packets);
	
	if (0 == strcmp(iface->name,"uplink"))
	{
		
		uint64 rx_bps = (stats.rx_bytes - zwl_stats_pre[0].rx_bytes)*8/5;
		uint64 rx_pps = (stats.rx_packets - zwl_stats_pre[0].rx_packets)/5;
		
		VLOG_DBG("zwl insert iface name %s statistics in db, stats.rx_bps=%lu, stats.rx_pps=%u", 
				iface->name, rx_bps, rx_pps);
	
		zwl_stats_pre[0].rx_bytes = stats.rx_bytes;
		zwl_stats_pre[0].rx_packets = stats.rx_packets;

	} else if (0 == strcmp(iface->name,"downlink")){
		uint64 rx_bps = (stats.rx_bytes - zwl_stats_pre[1].rx_bytes)*8/5;
		uint64 rx_pps = (stats.rx_packets - zwl_stats_pre[1].rx_packets)/5;
		
		VLOG_DBG("zwl insert iface name %s statistics in db, stats.rx_bps=%lu, stats.rx_pps=%u", 
				iface->name, rx_bps, rx_pps);
	
		zwl_stats_pre[1].rx_bytes = stats.rx_bytes;
		zwl_stats_pre[1].rx_packets = stats.rx_packets;
	}
...
/*******************************************************************************
 函数名称  :    refresh_controller_status
 功能描述  :    刷新控制器的状态
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    bridge_run__
 功能描述  :    启动网桥交换机，对应类型的datapath开始收包
 				1.运行每个类型的网桥的dp，dpdk 对应netdev类型
 				2.运行所有网桥对应of层交换机
...
bridge_run__(void)
{
    ...
    SSET_FOR_EACH (type, &types) {
		...
		/*让每一个类型的datapath运行，ovs-netdev类型的ofproto*/
    ...
    HMAP_FOR_EACH (br, node, &all_bridges) {
		...
		/*处理all_bridge上的每个bridge,让每一个bridge的ofproto交换机运行*/
...
/*******************************************************************************
 函数名称  :	bridge_run
 功能描述  :	初始化数据库中已经创建的虚拟网桥
 				网桥建立、配置和更新(vswitchd/bridge.c)
 				 0、读取数据库ovsdb中的配置到cfg, 此时, cfg中就有了诸如用户创建了多少个bridge, 每个bridge中有多少个port, iface等信息

				 1、初始化网桥的ofproto 层交换机：
				    a.主要调用bridge_init_ofproto()->ofproto_init()，且这个函数只执行一次，通过静态变量initialized控制。
				 
				      ①注册了变量ofproto_dpif_class（结构体ofproto_class，ofproto-dpif.c），这是ofproto的执行函数库的集合。

				        注册函数为ofproto_class_register(&ofproto_dpif_class)，将ofproto_dpif_class赋值给ofproto_classes[i]。
				        个人理解，i只可能为0（n_ofproto_classes只能为1），即ofproto_classes[0]就是ofproto_dpif_class（即ofproto的函数库）。
				        ofproto_dpif_class很有用，比如含有ofproto的运行、流表项的插入等操作函数，之后会用到。
				 
				        当然，注册了ofproto库，即ofproto_dpif_class之后，才可以创建ofproto。
				        结构体ofproto代表一个OpenFlow交换机，之后会在bridge_reconfigure()中调用ofproto_create()创建ofproto。
				 
				 2、运行网桥：bridge_run_()，主进行两项工作
				    1.是通过函数ofproto_type_run(type)让每一个类型的datapath运行
				    2.调用ofproto_run(br->ofproto)让每一个bridge的ofproto运行，ofproto的运行细节在后面三中着重分析。
...
void bridge_run(void)
{
	...
	/*另一个ovs进程启动着，删除本ovs进程的所有br及上面的port*/
	...
	/*初始化ofproto层交换机和port资源，只运行一次，如果运行过立即返回，
	  注册ofproto_dpif_class类，对ofproto交换机的操作函数*/
        ...
        if_notifier_changed(ifnotifier)) 
    {
		...
		/*ovs_cfg是从ovsdb-server里面读取出来的配置*/
...
/*******************************************************************************
 函数名称  :    bridge_collect_wanted_ports
 功能描述  :    网桥收集需要的端口填入wanted_ports
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    iface_get_requested_ofp_port
 功能描述  :    
 输入参数  :  	cfg---接口配置
 输出参数  :	
 返 回 值  : 	无
