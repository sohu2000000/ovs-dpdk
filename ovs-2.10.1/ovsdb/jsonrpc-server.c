...
/*ovsdb端的jsonrpc server*/

/*jsonrpc server session*/
/* JSON-RPC database server. */

struct ovsdb_jsonrpc_server {
    struct ovsdb_server up;
    unsigned int n_sessions;															/*server的session个数*/
    bool read_only;            /* This server is does not accept any
                                  transactions that can modify the database. */
    struct shash remotes;      /* Contains "struct ovsdb_jsonrpc_remote *"s. */			/*远端*/
};
 ...
 * session. */
struct ovsdb_jsonrpc_remote {
    struct ovsdb_jsonrpc_server *server;										/*server*/
    struct pstream *listener;   /* Listener, if passive. */						/*监听链接*/
    struct ovs_list sessions;   /* List of "struct ovsdb_jsonrpc_session"s. */	/*ovsdb server端的jsonrpc上的session*/
    uint8_t dscp;
    bool read_only;
    char *role;
};
...
/*******************************************************************************
 函数名称  :    ovsdb_jsonrpc_server_run
 功能描述  :    jsonrpc server run
 输入参数  :  	svr----server端的session
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
    ...
    SHASH_FOR_EACH (node, &svr->remotes) {
        ...
        if (remote->listener) {
            ...
            if (!error) {
				...
				/*为链接建立一个session结构，上面挂一个struct jsonrpc收发消息*/
...
struct ovsdb_jsonrpc_session {
    ...
    struct jsonrpc_session *js;  /* JSON-RPC session. */								/*jsonrpc的session*/
...
/*******************************************************************************
 函数名称  :    ovsdb_jsonrpc_session_run
 功能描述  :    处理session链接
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    ovsdb_jsonrpc_session_run_all
 功能描述  :    处理链接
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    ovsdb_jsonrpc_session_got_request
 功能描述  :    db 链接获取了请求消息
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
                                  ...
                                  struct jsonrpc_msg *request)
{
    ...
    if (reply) {
		...
		/*走session发出去，对jsonrpc_session_send 包了一层*/
...
/*******************************************************************************
 函数名称  :    ovsdb_jsonrpc_session_send
 功能描述  :    session发消息
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    ovsdb_jsonrpc_monitor_flush_all
 功能描述  :    
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
