...
struct action_fifo 
{
	...
	struct deferred_action fifo[DEFERRED_ACTION_FIFO_SIZE];                    /*不同执行动作队列*/
};

/*流key*/
struct action_flow_keys 
{
	struct sw_flow_key key[OVS_DEFERRED_ACTION_THRESHOLD];                     /*动作关键字2*/
};


/*每CPU变量动作执行fifo、动作流关键字*/
...
/*******************************************************************************
 函数名称  :    push_vlan
 功能描述  :    流表PUSH_VLAN动作会在数据包中添加对应的VLAN tag信息
 输入参数  :    vport---出端口
 				skb---要出的包
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :	do_output
 功能描述  :	调用do_output->ovs_vport_send->vport->ops->send发送函数
 				将数据包从output action对应的接口发送出去
 输入参数  :	skb---真实报文
				tun_info---通道信息
				key---skb提取的key
 输出参数  :	
...
/*******************************************************************************
 函数名称  :    ovs_vport_send
 功能描述  :    流表SET动作会修改数据包中指定的信息，如skb->priority skb->mark等信息
 输入参数  :    vport---出端口
 				skb---要出的包
 输出参数  :	
 返 回 值  : 	无
	     ...
	     a = nla_next(a, &rem)) 
	{
		...
		case OVS_ACTION_ATTR_OUTPUT: 
		{
			...
			if (nla_is_last(a, rem)) {
				...
				/*输出报文，为简化起见，本篇以output为例，其他action在后续分析*/
		...
		/*去Vlan tag, 流表POP_VLAN动作移除数据包中的Vlan tag信息并更新数据包中的校验和*/
...
/*******************************************************************************
 函数名称  :	ovs_execute_actions
 功能描述  :	循环遍历所有的flow action，执行相应的action动作，通过case语句，不同的action进行不同的操作
 输入参数  :    dp---端口所属网桥
 				skb---报文skb
 				acts---流表项的执行动作
 				key---skb提取的key
