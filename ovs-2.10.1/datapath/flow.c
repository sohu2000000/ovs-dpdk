...
/*******************************************************************************
 函数名称  :	key_extract
 功能描述  :	从flow提取key值
 				包括源MAC地址、目的MAC地址、VLAN信息、协议类型
 				源IP地址，目的IP地址、端口号等信息
 				所有的key值都可以从skb数据包中提取到
 输入参数  :	tun_info---通道信息
				key---填充key值
				skb---数据包指针
 输出参数  :	
			 ...
			 struct sk_buff *skb, struct sw_flow_key *key)
{
	...
	/*首先提取的是物理层的信息，主要是从哪个网口进入*/
	key->phy.priority = skb->priority; /*填充key值优先级端口掩码*/
	key->phy.in_port = OVS_CB(skb)->input_vport->port_no;/*设置key->phy.in_port为vport的接口序号*/
	key->phy.skb_mark = skb->mark;//默认为0  
	key->ovs_flow_hash = 0;
