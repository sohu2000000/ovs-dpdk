...
struct dp_packet {
...
#endif
    enum dp_packet_source source;  /* Source of memory allocated as 'base'. */		/*数据面收包报文存储类型，dpdk或其他*/
    ...
    union {
        struct pkt_metadata md;	   												/*数据面metadata*/
...
/*******************************************************************************
 函数名称  :    dp_packet_set_data
 功能描述  :    修改data 起始位置，左偏加上了tnl head
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    dp_packet_set_rss_hash
 功能描述  :    设置报文的rss 哈希值
 输入参数  :  	hash---miniflow计算的哈希值
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
...
/*******************************************************************************
 函数名称  :    dp_packet_has_flow_mark
 功能描述  :    判断报文是否带所属flow的mark,用于报文根据命中的flow导流到指定队列
 输入参数  :    p---批处理处理的报文
 		       mark---报文mark
 输出参数  :	
 返 回 值  : 	无
...
*******************************************************************************/
static inline bool
dp_packet_has_flow_mark(struct dp_packet *p OVS_UNUSED, uint32_t *mark OVS_UNUSED)
{

/*dpdk网卡设备*/
#ifdef DPDK_NETDEV
	/*dpdk mbuf 带有PKT_RX_FDIR_ID标记*/
...
{	
    size_t count;								 		 /* 当前批处理当前有多少个报文 */

    bool trunc; /* true if the batch needs truncate. */  /*批处理的报文是否需要截断*/
	
    struct dp_packet *packets[NETDEV_MAX_BURST]; 		 /*记录批处理报文指针，一次批量可以接收32个报文 */
};

/*******************************************************************************
 函数名称  :    dp_packet_batch_init
 功能描述  :    报文批处理初始化
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
*******************************************************************************/
static inline void
dp_packet_batch_add__(struct dp_packet_batch *batch, struct dp_packet *packet, size_t limit)
{
	/*当前队列报文批处理报文数未超限*/
...
/*******************************************************************************
 函数名称  :    dp_packet_batch_refill
 功能描述  :    pop完vxlan头的报文填回原来的报文patch批处理结构
 输入参数  :    batch---队列报文批处理
 			    packet---pop完vxlan头的报文
 			    idx---报文在队列的index
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
...
*******************************************************************************/
static inline void
dp_packet_batch_refill(struct dp_packet_batch *batch, struct dp_packet *packet, size_t idx)
{
    dp_packet_batch_add__(batch, packet, MIN(NETDEV_MAX_BURST, idx + 1));
}

/*******************************************************************************
 函数名称  :    dp_packet_batch_init_packet
 功能描述  :    报文批处理结构初始化
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
*******************************************************************************/
static inline void
dp_packet_batch_clone(struct dp_packet_batch *dst, struct dp_packet_batch *src)
{
    ...
    DP_PACKET_BATCH_FOR_EACH (i, packet, src) {
    	/*每个报文clone到新buffer，记录入out 批处理结构*/
...
/*******************************************************************************
 函数名称  :    dp_packet_batch_apply_cutlen
 功能描述  :    使用报文尾巴需要cut掉新的长度
 输入参数  :  	batch---报文批处理
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    dp_packet_batch_reset_cutlen
 功能描述  :    cutlen全部设置成0 
 输入参数  :  	batch---报文批处理
 输出参数  :	
 返 回 值  : 	无
