...
struct netdev_dpdk {
    PADDED_MEMBERS_CACHELINE_MARKER(CACHE_LINE_SIZE, cacheline0,
        dpdk_port_t port_id;                                                        /*dpdk 层端口id*/

        /* If true, device was attached by rte_eth_dev_attach(). */
        bool attached;
        /* If true, rte_eth_dev_start() was successfully called */
        bool started;
        struct eth_addr hwaddr;                                                    /*硬件mac*/
        int mtu;
        int socket_id;
        int buf_size;
        int max_packet_len;
        ...
        struct dpdk_tx_queue *tx_q;                                                     /*发队列*/
        struct rte_eth_link link;
    );

    PADDED_MEMBERS_CACHELINE_MARKER(CACHE_LINE_SIZE, cacheline1,
        struct ovs_mutex mutex OVS_ACQ_AFTER(dpdk_mutex);
        struct dpdk_mp *dpdk_mp;                                                        /*内存池*/
    ...
    PADDED_MEMBERS(CACHE_LINE_SIZE,
        ...
        int requested_mtu;
        int requested_n_txq;                /*ovs配置的txq个数*/
        int requested_n_rxq;                /*ovs配置的rxq个数*/
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_mempool_configure
 功能描述  :    emc流表删除
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    dpdk_eth_dev_port_config
 功能描述  :    端口队列配置
 输入参数  :  	dev---端口对应netdev_dpdk结构
                n_rxq---端口配置的rxq
                n_txq---端口配置的txq
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    dpdk_eth_dev_init
 功能描述  :    dpdk接管的端口初始化
 输入参数  :  	dev---netdev对应的dpdk结构
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
/*******************************************************************************
 函数名称  :    common_construct
 功能描述  :    构造端口对应的netdev
 输入参数  :  	netdev---端口对应netdev
                port_no---DPDK_ETH_PORT_ID_INVALID 最大32个口
                type---DPDK_DEV_ETH
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_get_port_by_mac
 功能描述  :    根据mac获取port
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_process_devargs
 功能描述  :    dpdk参数处理
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
                            ...
                            const char *devargs, char **errp)
{
    ...
    if (strncmp(devargs, "class=eth,mac=", 14) == 0) {
		...
		/*从dpdk获取端口*/
        if (rte_eth_dev_get_port_by_name(name, &new_port_id) || !rte_eth_dev_is_valid_port(new_port_id)) {

			/*dpdk没有获取到，attach 到dpdk，返回新port id*/
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_vhost_rxq_recv
 功能描述  :    调用dpdk接口rte_vhost_dequeue_burst接收报文
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
                           ...
                           struct dp_packet_batch *batch, int *qfill)
{
	...
	/*调用dpdk接口rte_vhost_dequeue_burst接收报文,dpdk接收报文的接口*/
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_rxq_recv
 功能描述  :    dpdk netdev收包流程，struct netdev_class dpdk_class
 输入参数  :  	rxq---收包队列
 				batch---报文batch, dpdk收上来放入batch
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
                         ...
                         struct dp_packet **pkts, int cnt)
{
    ...
    do {
		...
		1.预取vhost_virtqueue的信息
		2.根据发送报文数count，将报文放入ring当中。此处涉及到一次数据拷贝
		3.内存屏障进行同步。
        4.如果对方支持中断，通过eventfd_write通知对方有报文了，一般是对方virtio有中断，virtio-user无中断*/
...
*******************************************************************************/
static inline void
netdev_dpdk_send__(struct netdev_dpdk *dev, int qid, struct dp_packet_batch *batch, bool concurrent_txq)
{
    if (OVS_UNLIKELY(!(dev->flags & NETDEV_UP))) 
	{
        dp_packet_delete_batch(batch, true);
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_get_stats
 功能描述  :    获取流量
 输入参数  :  	netdev---端口netdev设备
 				stats---ovs端口netdev的流量
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_class_init
 功能描述  :    dpdk netdev类初始化
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_ring_construct
 功能描述  :    ring 构建
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    dpdk_vhost_reconfigure_helper
 功能描述  :    vhost 重配
 输入参数  :  	dev---
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    ufid_to_rte_flow_find
 功能描述  :    根据uifd查全局ufid flow map ufid_to_rte_flow rte_flow是否存在
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    ufid_to_rte_flow_associate
 功能描述  :    ufid 关联到rte flow 记录入全局ufid rte flow关系图
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
struct flow_patterns {
    struct rte_flow_item *items;		/*所有的match 条件*/
    int cnt;							/*match 条件个数*/
    int current_max;					/*当前支持的最大item 个数*/
};

/*流表action pattern*/
struct flow_actions {
    struct rte_flow_action *actions;	/*所有的action*/
    int cnt;							/*action个数*/
    int current_max;					/*当前支持的action 最大个数*/
};
...
/*******************************************************************************
 函数名称  :    add_flow_pattern
 功能描述  :    rte_flow match添加
 输入参数  :  	patterns---match条件
 				type---match条件item类型，字段类型
 				spec---match字段的结构体地址
 				mask---match mask字段的结构体地址
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
add_flow_pattern(struct flow_patterns *patterns, enum rte_flow_item_type type,
	...
	/*记录match 类型，match、match mask临时存储结构体地址*/
...
/*******************************************************************************
 函数名称  :    add_flow_action
 功能描述  :    emc流表删除
 输入参数  :  	actions---action内存
 				conf---新增action item
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_add_rte_flow_offload
 功能描述  :    下发rte_flow
 输入参数  :  	netdev
 				match---match条件，offload->match
 				actions---actions动作，offload->action
 				actions_len---动作长度
 				ufid---流表ufid
 				info---offload信息
 输出参数  :	
...
netdev_dpdk_add_rte_flow_offload(struct netdev *netdev,
	...
	/*match 提取mac 填到eth*/
	if (!eth_addr_is_zero(match->wc.masks.dl_src) || !eth_addr_is_zero(match->wc.masks.dl_dst)) 
    {
    	/*rte flow mac*/
		...
		/*eth_spec 地址记录入rte flow match pattern*/
	...
	if (match->wc.masks.vlans[0].tci && match->flow.vlans[0].tci) 
	{
		...
		/*vlan字段结构体地址记录入pattern*/
        add_flow_pattern(&patterns, RTE_FLOW_ITEM_TYPE_VLAN, &vlan_spec, &vlan_mask);
    }
	...
	if (match->flow.dl_type == htons(ETH_TYPE_IP)) 
	{
		...
		/*ipv4_spec记录入pattern*/
        add_flow_pattern(&patterns, RTE_FLOW_ITEM_TYPE_IPV4,  &ipv4_spec, &ipv4_mask);
	...
	if (proto == IPPROTO_TCP) 
	{
		...
		/*tcp_spec、tcp_mask记录入入pattern*/
        add_flow_pattern(&patterns, RTE_FLOW_ITEM_TYPE_TCP, &tcp_spec, &tcp_mask);
    ...
    if (proto == IPPROTO_UDP) 
	{
		...
		/*udp_spec、udp_mask结构地址记录入pattern*/
        add_flow_pattern(&patterns, RTE_FLOW_ITEM_TYPE_UDP, &udp_spec, &udp_mask);
	...
	if (proto == IPPROTO_ICMP) {
		...
		/*icmp字段填入pattern*/
        add_flow_pattern(&patterns, RTE_FLOW_ITEM_TYPE_ICMP, &icmp_spec, &icmp_mask);
...
end_proto_check:
	...
	/*调dpdk的库下发rte flow 返回创建的rte flow*/
    flow = rte_flow_create(dev->port_id, &flow_attr, patterns.items, actions.actions, &error);
    free(rss);
	...
	/*ufid 关联到rte flow 记录入全局ufid rte flow关系图*/
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_validate_flow
 功能描述  :    校验match条件
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    netdev_dpdk_destroy_rte_flow
 功能描述  :    rte flow 存在先删除
 输入参数  :  	dev---dev
 				ufid---rte flow的ufid
 				rte_flow---rte flow
...
netdev_dpdk_destroy_rte_flow(struct netdev_dpdk *dev,
	...
	/*删除 rte flow 在其他flow依赖它时可能会删除失败*/
    ret = rte_flow_destroy(dev->port_id, rte_flow, &error);
    if (ret == 0) {

		/*flow ufid 填入全局ufid 和flow记录表ufid_to_rte_flow*/
                     ...
                     struct dpif_flow_stats *stats OVS_UNUSED) 
{
	...
	/*ufid对应的rte_flow 是否存在*/
    rte_flow = ufid_to_rte_flow_find(ufid);
    if (rte_flow) 
	{
		/*存在先删除*/
        ret = netdev_dpdk_destroy_rte_flow(netdev_dpdk_cast(netdev), ufid, rte_flow);
        if (ret < 0) 
		{
            return ret;
        }
	...
	/*下发rte_flow*/
    return netdev_dpdk_add_rte_flow_offload(netdev, match, actions, actions_len, ufid, info);
}

/*******************************************************************************
 函数名称  :    netdev_dpdk_flow_del
 功能描述  :    
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
netdev_dpdk_flow_del(struct netdev *netdev, const ovs_u128 *ufid,
	...
	/*根据uifd查全局ufid flow map ufid_to_rte_flow rte_flow是否存在*/
...
/*dpdk 的netdev操作类，调用op操作对应类型的网卡*/
static const struct netdev_class dpdk_class =
    NETDEV_DPDK_CLASS(
        "dpdk",
        netdev_dpdk_class_init,									/*dpdk*/
