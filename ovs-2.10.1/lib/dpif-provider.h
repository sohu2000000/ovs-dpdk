...
/*ovs 数据面接口*/
struct dpif {
    //const struct dpif_class *dpif_class; /*数据接口类*/
	const struct dpif_class *dpif_class_zwl; /*数据接口类*/
    char *base_name;				 	 /*dp name*/
    char *full_name;					 /*dp name 带type*/
...
struct dpif_flow_dump {
    //struct dpif *dpif;													    /**/
    struct dpif *zwl_dpif;													    /*dpif 结构*/
	bool terse;         /* If true, key/mask/actions may be omitted. */			/*设置true 返回的flow 忽略key/mask/actions*/
};
...
struct dpif_flow_dump_thread {
    struct dpif *dpif;					/*dpif结构*/
};

/*******************************************************************************
 函数名称  :    dpif_flow_dump_thread_init
 功能描述  :    dump线程结构初始化
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
