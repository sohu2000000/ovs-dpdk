 ...
 *
 *    - S_VOID: No connection information is configured.							没有配置链接配置
 *
 *    - S_BACKOFF: Waiting for a period of time before reconnecting.                重连前等待状态
 *
 *    - S_CONNECTING: A connection attempt is in progress and has not yet           链接中
 *      succeeded or failed.
 *
 *    - S_ACTIVE: A connection has been established and appears to be healthy.      链接建立了且是健康的
 *
 *    - S_IDLE: A connection has been established but has been idle for some        链接建立了但是空跑状态，或者echo 已经发了但还没有收到回应
 *      time.  An echo request has been sent, but no reply has yet been
 *      received.
 *
 *    - S_DISCONNECTED: An unreliable connection has disconnected and cannot be		不可靠的链接已断开且不能自动重连
...
struct rconn {
    ...
    enum state state;			/*链接状态*/
    time_t state_entered;

    struct vconn *vconn;		/*rconn对应的虚拟链接*/
    char *name;                 /* Human-readable descriptive name. */		/*描述一条链接的name*/
...
/*******************************************************************************
 函数名称  :    rconn_connect_unreliably
 功能描述  :    rconn 链接器vconn链接
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    rconn_run
 功能描述  :    rconn_run-->vconn_run-->vconn_stream_run-->stream_send-->fd_send最终发送报文。
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
    ...
    OVS_EXCLUDED(rc->mutex)
{
    ...
    if (rc->vconn) {
		...
		/*获取虚拟链接状态，非正常断开，例如变化端口，broken pipe*/
	...
	/*monitor连接后，复制得到的openflow消息发送回ofctl客户端 并将监测复制得到的of消息接收到相应rc->n_monitors[i]实例中*/
