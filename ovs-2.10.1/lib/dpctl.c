...
struct dpctl_command {
    const char *name;					/*命令name "add-flow"*/
    const char *usage;					/*用法 "[dp] flow actions"*/
    int min_args;						/*最小参数个数2*/
    int max_args;						/*最大参数个数3*/
    dpctl_command_handler *handler;		/*命令处理回调dpctl_add_flow*/
    enum { DP_RO, DP_RW} mode;			/*读写DP_RW*/
};
...
/*******************************************************************************
 函数名称  :  parsed_dpif_open
 功能描述  :  打开
 输入参数  :  arg_---dp name, netdev@br0
 			  create---false
 			  dpifp---dpctl 参数
...
static int
opt_dpif_open(int argc, const char *argv[], struct dpctl_params *dpctl_p, int max_args, struct dpif **dpifp, int *indexp)
{
    char *dpname;
	...
	{
		/*获取datapath name，第一个参数类型 add-flow  br-x*/
...
*******************************************************************************/
static int
dpctl_add_if(int argc OVS_UNUSED, const char *argv[], struct dpctl_params *dpctl_p)
{
    struct dpif *dpif;
...
dpctl_dump_flows(int argc, const char *argv[], struct dpctl_params *dpctl_p)
{
    ...
    if (filter) 
	{
		...
		/*执行查询*/
		char *err = parse_ofp_exact_flow(&flow_filter, &wc_filter, NULL, filter, &port_map);
	...
	while (dpif_flow_dump_next(flow_dump_thread, &f, 1)) 
	{
		...
		if (!type || flow_passes_type_filter(&f, type)) 
		{
			...
			VLOG_DBG("zwl dpctl_dump_flows ds_cstr(&ds)=%s",ds_cstr(&ds));
        }
    }
...
/*******************************************************************************
 ...
 输入参数  :  argc---参数个数
 			  argv---参数数组
 			  flags----DPIF_FP_CREATE flow的add  mod 等
 			  dpctl_p---dpctl参数
...
*******************************************************************************/
static int
dpctl_put_flow(int argc, const char *argv[], enum dpif_flow_put_flags flags, struct dpctl_params *dpctl_p)
{
	/*match数组*/
	...
	VLOG_DBG("zwl flow dpctl_put_flow 111");


    ufid_present = false;

	/*如果flow命令行存在 提取ufid、返回ufid字符串长度*/
	...
	/*遍历dp接口上 端口name填入port_names hmap*/
    DPIF_PORT_FOR_EACH (&dpif_port, &port_dump, dpif) 
	{
	...
	/*match mask存储结构内存申请、初始化*/
    ofpbuf_init(&mask, 0);

	/*flow各字段解析提取填入key 、mask*/
	...
	VLOG_DBG("zwl flow dpctl_put_flow 222");

    /* The flow will be added on all pmds currently in the datapath. */
	/*最终的下流表*/
    error = dpif_flow_put(dpif, flags, key.data, key.size, mask.size == 0 ? NULL : mask.data, mask.size, actions.data, 
    			actions.size, ufid_present ? &ufid : NULL, PMD_ID_NULL, dpctl_p->print_statistics ? &stats : NULL);
    if (error) 
	{
	...
	VLOG_DBG("zwl flow dpctl_put_flow 333");
...
{
	VLOG_DBG("zwl flow dpctl_add_flow");
...
/*******************************************************************************
 函数名称  :    dpctl_unixctl_handler
 功能描述  :    dpctl命令处理函数
 输入参数  :  	conn---rpc链接
 输出参数  :	argc---参数个数 3
 				argv---参数 "dpctl/add-flow match action"
 返 回 值  : 	无
-------------------------------------------------------------------------------
 最近一次修改记录 :
 修改作者	:	
                      ...
                      void *aux)
{
	...
	VLOG_DBG("zwl dpctl_unixctl_handler argc=%d\n",argc);

	for(int i = 0; i < argc; i++ )
	{
		VLOG_DBG("zwl dpctl_unixctl_handler argv=%s\n",argv[i]);
	}
    ...
    while (argc > 1 && !error) 
	{
		...
		VLOG_DBG("zwl dpctl_unixctl_handler argv=%d\n",argv[i]);
		...
		else if (arg[0] == '-' && arg[1] != '\0') 
		{
			...
			VLOG_DBG("zwl dpctl_unixctl_handler - argv[1]=%s\n", argv[1]);
        } 

		/*不带选项*/
		else 
		{
			VLOG_DBG("zwl no op dpctl_unixctl_handler \n");
    ...
    if (!error) {
		 ...
		 VLOG_DBG("zwl dpctl_unixctl_handler no error");
		 
		/*获取传过来的参数*/
		dpctl_command_handler *handler = (dpctl_command_handler *) aux;

		/*调用回调函数 dpctl_add_flow dpctl_dump_flows*/
		error = handler(argc, argv, &dpctl_p) != 0;
    }

    if (error) {
		VLOG_DBG("zwl dpctl_unixctl_handler has error");
...
/*******************************************************************************
 函数名称  :    dpctl_unixctl_register
 功能描述  :    dpctl命令行注册 
 				struct dpctl_command all_commands[] 注册到了unix
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
