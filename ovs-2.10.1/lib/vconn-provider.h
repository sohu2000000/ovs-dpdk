...
/*活跃的虚拟链接 链接的是openflow 设备*/
/* Active virtual connection to an OpenFlow device. */

/* This structure should be treated as opaque by vconn implementations. */
struct vconn {
    const struct vconn_class *vclass;  /*虚拟链接类,打开、收、发等*/
    int state;						   /*链接的状态*/
    int error;

    /* OpenFlow versions. */
    uint32_t allowed_versions;  /* Bitmap of versions we will accept. */			/*允许的openflow协议类型*/
    uint32_t peer_versions;     /* Peer's bitmap of versions it will accept. */		/*对端允许的协议类型*/
    enum ofp_version version;   /* Negotiated version (or 0). */					/*协商的协议类型*/
    bool recv_any_version;      /* True to receive a message of any version. */		/*是否允许任意类型的协议*/
