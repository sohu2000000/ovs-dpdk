...
/*两种provider system 和netdev的类，对应两种datapath类型*/
...
/*******************************************************************************
 函数名称  :    dp_register_provider__
 功能描述  :    注册dp的provider
 输入参数  :  	new_class---dpdk的 dpif层操作类
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
/*******************************************************************************
 函数名称  :    dp_enumerate_types
 功能描述  :    枚举数据面的所有类型添加到type hmap
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    dp_enumerate_names
 功能描述  :    根据类型查找dp 类，根据dp类获取dp name
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :  dp_parse_name
 功能描述  :  解析数据面name
 输入参数  :  datapath_name_---netdev@br0
...
/*******************************************************************************
 ...
 功能描述  :  
 输入参数  :  dpifp---记录新建的dp
 			  name---dp name，backer的name ovs-netdev，网桥name br0
 			  type---dp的type，dpdk为netdev
 			  create---false
 输出参数  :  
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
do_open(const char *name, const char *type, bool create, struct dpif **dpifp)
{
	...
	/*根据dp类型查找dp 类结构，用户态为netdev，内核态为system*/
    registered_class = dp_class_lookup(type);
    if (!registered_class) 
	{
	...
	/*调用dp类结构的操作函数，新建或打开对应类型dp_netdev dpif_netdev_open，获取对应dpif*/
    ...
    if (!error) 
	{
        ...
        DPIF_PORT_FOR_EACH(&dpif_port, &port_dump, dpif) 
        {
			...
			/*根据端口name查询端口netdev，tap 端口创建对应的端口*/
...
/*******************************************************************************
 函数名称  :    dpif_create_and_open
 功能描述  :    创建或直接打开一个dp，根据dp类型和name
 输入参数  :  	name---backer name ovs-netdev，根据backername 找dp_netdev
 				type---netdev
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    dpif_run
 功能描述  :    启动dp 交换机 dp起pmd线程开始收包(pmd上端口rxq)  dp_netdev网络设备，创建网桥时创建
 输入参数  :  	dpif---dp层的接口抽象
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
dpif_run(struct dpif *dpif)
{
    ...
    if (dpif->dpif_class->run) {
		...
		/*dpif层交换机初始化 dpif_netdev_run 启动pmd线程 开始收包*/
...
dpif_port_add(struct dpif *dpif, struct netdev *netdev, odp_port_t *port_nop)
{
	...
	VLOG_DBG("dpif_port_add port_no=%u",port_no);

	/*数据面添加端口 dpif_netdev_port_add*/
    error = dpif->dpif_class->port_add(dpif, netdev, &port_no);
    if (!error) 
	{
        VLOG_DBG_RL(&dpmsg_rl, "%s: added %s as port %"PRIu32,dpif_name(dpif), netdev_name, port_no);
                     ...
                     const struct smap *cfg)
{
    ...
    if (dpif->dpif_class->port_set_config) {
		...
		/*设置端口的rxq cpu亲和性 dpif_netdev_port_set_config*/
...
/*******************************************************************************
 ...
 功能描述  :    流表hash算uuid
 输入参数  :  	dpif---dp接口描述结构
 				key---match.flow---struct flow flow---match填入的flow
 				key_len---match的长度
 				hash---ufid
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 ...
 功能描述  :    下流表
 输入参数  :    dpif---dp结构
   				flags----DPIF_FP_CREATE、flow 操作类型
 			    dpctl_p---dpctl参数
 			    key---match描述符、描述match内存
 			    key_len---match里match条件内存字节数
 			    mask---match mask描述符、描述match mask内存
 			    mask_len---mask 条件内存字节数
 			    actions---action 描述符、描述action内存
 			    actions_len---action内存字节数
 			    stats---flow的流量统计
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    dpif_flow_dump_thread_create
 功能描述  :    返回dump flow使用的结构
 输入参数  :  	dump---dump flow
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
{
	/*dpif_netdev_class dpif_netdev_flow_dump_thread_create*/
    return dump->dpif->dpif_class->flow_dump_thread_create(dump);
}

/*******************************************************************************
 函数名称  :    dpif_flow_dump_thread_destroy
 功能描述  :    释放thread结构
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    dpif_flow_dump_next
 功能描述  :    dump pmd上flow 每次最多dump 50条flow
 输入参数  :  	thread---struct dpif_flow_dump_thread  dump flow用到的线程结构
 				flows---记录dump到的dpif flow
 				max_flows---每次最大dump 50条flow
 输出参数  :	
 返 回 值  : 	返回dump到的flow个数
--------------------------------------------------------------------------------
 最近一次修改记录 :
 修改作者	:	
 修改目的	: 	
                    ...
                    struct dpif_flow *flows, int max_flows)
{
	...
	/*dump所有的flow dpif_netdev_flow_dump_next，dump到的flow记录到flows，每次dump最多50条flow*/
...
{
	VLOG_DBG("zwl flow dpif_operate 111");
    ...
    while (n_ops > 0) 
	{
        ...
        for (chunk = 0; chunk < n_ops; chunk++) 
		{
			...
			/*op 类型执行时存在需要帮助的op break出去*/
            if (op->type == DPIF_OP_EXECUTE && dpif_execute_needs_help(&op->execute)) 
            {
                break;
            }
        }
        ...
        if (chunk) 
		{
			...
			VLOG_DBG("zwl flow dpif_operate 222");
			
			/*调用dpif层op函数操作流表 dpif_netdev_operate */
            dpif->dpif_class->operate(dpif, ops, chunk);

			/*遍历要op的流表数，打印对应的流表操作日志*/
            for (i = 0; i < chunk; i++) 
			{
...
/*******************************************************************************
 函数名称  :    dpif_meter_set
 功能描述  :    申请dp层meter资源关联到dp
 输入参数  :  	dpif---交换机dpif层
 输出参数  :	meter_id---meter流表使用的meter id
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
 修改作者	:	
               ...
               struct ofputil_meter_config *config)
{
	...
	#if 1
	VLOG_DBG("zwl:dpif_meter_set->dpif_netdev_meter_set:%s: DPIF meter %"PRIu32" set",
                    dpif_name(dpif), meter_id.uint32);
	#endif
	

	/*软件meter下发 dpif_netdev_meter_set  申请dp层meter资源关联到dp*/
...
#if 1
	VLOG_DBG("zwl-dpif-delmeterflow dpif_meter_del meter_id=%u.", meter_id);
#endif

	/*dpif层 meter资源删除 dpif_netdev_meter_del*/
