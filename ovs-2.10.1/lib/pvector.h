...
{
    int priority;	/*掩码的优先级，pvector_impl中的vector是按照priority从小到大排序的 */
    void *ptr;	    /*指向了struct dpcls_subtable结构 */
};
...
struct pvector_impl 
{
    ...
    struct pvector_entry vector[];/* 优先级，初始化的时候只有4个元素.后续可能会扩充 */
};
...
struct pvector 
{
    OVSRCU_TYPE(struct pvector_impl *) impl;  /* 指向具体子表信息 */
    struct pvector_impl *temp;   			  /*优先级 平时 temp都是为NULL.只有当pvector扩充时,temp才用来临时缓存数据.待排好序后,再拷贝到impl中,temp再置NULL */
};
