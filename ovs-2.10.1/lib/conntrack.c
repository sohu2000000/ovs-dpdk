...
conntrack_init(struct conntrack *ct)
{
	...
	/*遍历 ct bucket 1<<8 个bucket 处理上面超时的conn*/
...
/*******************************************************************************
 函数名称  :    sweep_bucket
 功能描述  :    从ct bucket上删除超时的conn，返回现有链接最早的超时时间，返回值可能不是一个未来的时间，比当前时间还小，因为到了要返回的限制数
 输入参数  :  	ct---链接跟踪
 				ctb---ct下bucket
 				now---当前时间
 				limit---最大要老化的链接数
 输出参数  :	
 返 回 值  : 	返回bucket 超时链表上，最早要超时的链接的超时时间
--------------------------------------------------------------------------------
 最近一次修改记录 :
 修改作者	:	
 修改目的	: 	
...
/*******************************************************************************
 函数名称  :    conntrack_clean
 功能描述  :    清理老的链接，返回下次超时时间
 输入参数  :  	ct---dp ct链接跟踪
 				now---当前时间
 输出参数  :	
 返 回 值  : 	无
...
conntrack_clean(struct conntrack *ct, long long now)
{
    ...
    for (unsigned i = 0; i < CONNTRACK_BUCKETS; i++) {
		...
		/*如果链接在bucket上分布比较均匀，最大限制10%超时，剩下下次删除*/
...
/*******************************************************************************
 函数名称  :    expectation_clean
 功能描述  :    清除ct下conn链接
 输入参数  :  	ct---ct结构
 				master_key---要删除的conn 的key
 				basis---conn所在的bucket
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
