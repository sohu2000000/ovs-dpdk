...
struct cmap_bucket 
{
    ...
    PADDED_MEMBERS(CACHE_LINE_SIZE,
        ...
        struct cmap_node nodes[CMAP_K];  /*哈希桶节点，链表结构，包含所有端口对应struct dpcls_rule*/
...
struct cmap_impl 
{
    PADDED_MEMBERS_CACHELINE_MARKER(CACHE_LINE_SIZE, cacheline0,
        unsigned int n;             /* Number of in-use elements. */		/*在使用中的元素*/
        unsigned int max_n;         /* Max elements before enlarging. */	/*扩展前最大元素个数*/
        unsigned int min_n;         /* Min elements before shrinking. */	/*收缩前最大元素个数*/
        uint32_t mask;              /* Number of 'buckets', minus one. */   /*桶深*/
        uint32_t basis;             /* Basis for rehashing client's         /*拓展、收缩步长*/
                                    //hash values.
    );

	 																				/*cmap哈希桶结构，挂不同端口的dpcls结构*/
 ...
 * Return node for the entry of index or NULL if the index beyond boundary */
 /*******************************************************************************
 函数名称  :  cmap_find_by_index
 功能描述  :  根据index 返回smc流表项
 输入参数  :  cmap---emc真实流表
 			  index---smc流表项 index
 输出参数  :  output_cnt
 返 回 值  : 	无
...
*******************************************************************************/
unsigned long
cmap_find_batch(const struct cmap *cmap, unsigned long map, uint32_t hashes[], const struct cmap_node *nodes[])
{
    const struct cmap_impl *impl = cmap_get_impl(cmap);
...
/*******************************************************************************
 函数名称  :    cmap_insert
 功能描述  :    emc流表插入
 输入参数  :  	cmap---pmd->flow_table  流表cache
 				node---流表节点 &flow->node 
 				hash---根据uuid算的hash,
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    cmap_next_position
 功能描述  :    获取下一个node
 输入参数  :  	pos---dump开始的位置，开始的bucket、node、链表偏移等
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
            ...
            for (i = 0; node; i++, node = cmap_node_next(node)) {
				...
				if (i == offset) {
                    ...
                    if (cmap_node_next(node)) {
                        ...
                        /*没有获取到node，node下已没有flow，entry涨一次*/
                        entry++;
                        offset = 0;
                    }
