...
struct netdev_saved_flags {
    struct netdev *netdev;			/*网络设备结构*/
    struct ovs_list node;           /* In struct netdev's saved_flags_list. */
    enum netdev_flags saved_flags;	/*网卡的模式*/
...
struct netdev_registered_class {
    ...
    const struct netdev_class *class;												/*netdev的操作类*/
...
/*******************************************************************************
 函数名称  :    netdev_has_tunnel_push_pop
 功能描述  :    netdev存在隧道push pop操作
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
    ...
    OVS_EXCLUDED(netdev_mutex)
{
	...
	/*遍历所有类型netdev 类调用run接口 dpdk 接口调用 static const struct netdev_class dpdk_class*/
    CMAP_FOR_EACH (rc, cmap_node, &netdev_classes) {
        if (rc->class->run) {

			/*struct netdev_class dpdk_class netdev_dpdk_run 没有此函数 */
			/*netdev_linux_run linux*/
...
/*******************************************************************************
 函数名称  :    netdev_lookup_class
 功能描述  :    查找对应类型的netdev对应的类
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 ...
 * */
 /*******************************************************************************
 函数名称  :    netdev_open
 功能描述  :    打开设备，端口的netdev设备
 输入参数  :  	name---端口name，ovs-netdev
 				type---端口netdev的类型，"system", "tap", "gre"
 				netdevp---端口信息赋值给端口的netdev
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
    ...
    OVS_EXCLUDED(netdev_mutex)
{
	...
	/*全局结构netdev_shash 根据端口name查找端口ovs-netdev是否已存在*/
    netdev = shash_find_data(&netdev_shash, name);

	/*查找到端口ovs-netdev*/
    if (netdev && type && type[0] && strcmp(type, netdev->netdev_class->type)) 
	{
	...
	/*没有查到ovs-netdev 端口netdev*/
    if (!netdev) 
	{
		/*对应netdev操作类*/
        struct netdev_registered_class *rc;

		/*根据类查找netdev操作类，dpdk类型的用户态datapath为netdev类型，ovs-netdev端口类型为tap类型，一个backer创建一个，为啥是tap类型*/
        rc = netdev_lookup_class(type && type[0] ? type : "system");

		/*找到*/
        if (rc && ovs_refcount_try_ref_rcu(&rc->refcnt)) 
		{
			/*端口netdev 内存申请 netdev_dpdk_alloc, static const struct netdev_class dpdk_class*/
            ...
            if (netdev) 
			{
				...
				/*netdev构造 netdev_linux_construct_tap，为什么，会构造对应的netdev*/
				/*netdev_dpdk_construct 构造dpdk端口对应的netdev*/
				...
				else
				{
					...
					/*netdev_dpdk_dealloc 释放netdev内存*/
    ...
    OVS_EXCLUDED(netdev_mutex)
{
    if (netdev->netdev_class->set_config) {
		...
		/*端口设置 netdev_dpdk_set_config 设置端口rxq个数等*/
...
/*******************************************************************************
 函数名称  :    netdev_rxq_open
 功能描述  :    打开netdev接收队列
 输入参数  :  	netdev---port netdev结构
 				rxp---接收队列指针数组
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :  netdev_rxq_recv
 功能描述  :  接收队列接收报文
 输入参数  :  rx---接收队列
 			  batch---队列批处理结构
 			  qfill--int变量指针，用于获取报文个数
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
...
*******************************************************************************/
int
netdev_rxq_recv(struct netdev_rxq *rx, struct dp_packet_batch *batch, int *qfill)
{
    int retval;

	/*从队列收包、放入批处理结构*/     /*dpdk队列收包函数struct netdev_class netdev_dpdk_rxq_recv，port添加时会初始化port使用的rx队列*/
...
/*******************************************************************************
 函数名称  :    netdev_set_tx_multiq
 功能描述  :    netdev设置多发送队列
 输入参数  :  	netdev---端口对应dp netdev结构
 				n_txq---发送队列个数=线程数
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
...
netdev_set_tx_multiq(struct netdev *netdev, unsigned int n_txq)
{
	...
	/*netdev设置多发送队列*/
    error = (netdev->netdev_class->set_tx_multiq ? netdev->netdev_class->set_tx_multiq(netdev, MAX(n_txq, 1)) : EOPNOTSUPP);
...
/*******************************************************************************
 函数名称  :  netdev_send
 功能描述  :  OVS流表output action 发送数据包时的函数
 输入参数  :  netdev---发包端口对应结构，p->port->netdev
 			  qid---发队列ID
 			  batch---发队列批处理结构
 			  vport---出端口
 			  skb---要出的包
...
{
	/*最后调用dev_queue_xmit发送数据包 netdev_dpdk_send__*/
    int error = netdev->netdev_class->send(netdev, qid, batch, concurrent_txq);
    if (!error) 
	{
...
/*******************************************************************************
 函数名称  :    netdev_pop_header
 功能描述  :    解vxlan头，解完报文重新填回batch
 输入参数  :  	netdev---vxlan in_port
 				batch---报文批处理结构
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    netdev_push_header
 功能描述  :    报文添加vxlan头
 输入参数  :  	netdev---vxlan口对应netdev
 				batch---封vxlan批处理报文
 				data---封vxlan属性数据
 输出参数  :	
 返 回 值  : 	无
                   ...
                   const struct ovs_action_push_tnl *data)
{
    ...
    {
		/*报文添加vxlan头 netdev_tnl_push_udp_header*/
...
/*******************************************************************************
 函数名称  :    netdev_get_ifindex
 功能描述  :    获取端口netdev index
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
*******************************************************************************/
int
netdev_turn_flags_on(struct netdev *netdev, enum netdev_flags flags, struct netdev_saved_flags **sfp)
{
...
/*******************************************************************************
 函数名称  :    netdev_get_stats
 功能描述  :    netdev 获取流量
 输入参数  :  	netdev---端口对应netdev
 				stats---流量统计
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
netdev_get_stats(const struct netdev *netdev, struct netdev_stats *stats)
{
	...
	/*获取流量 dpdk获取的接口流量赋值netdev netdev_dpdk_get_stats*/
...
/*******************************************************************************
 函数名称  :    netdev_get_type
 功能描述  :    获取netdev的类型，"system", "tap", "gre"
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    netdev_is_reconf_required
 功能描述  :    需要重配置标记
 输入参数  :  	netdev---端口对应netdev
 输出参数  :	
 返 回 值  : 	无
...
{
	/*重新配置序列号与上次配置序号不相等的端口，新端口序号都不一致*/
...
netdev_reconfigure(struct netdev *netdev)
{
	...
	/*调用端口重配函数  dpdk类型端口重配netdev_dpdk_reconfigure*/
                ...
                struct dpif_flow_stats *stats)
{
	...
	VLOG_DBG("netdev_flow_put dpdk offload flow put to rte_flow !\n");

	/*流表下发 dpdk netdev_dpdk_flow_put*/
    return (class->flow_put ? class->flow_put(netdev, match, actions, act_len, ufid, info, stats) : EOPNOTSUPP);
}
                ...
                struct dpif_flow_stats *stats)
{
	...
	/*流表下发 dpdk netdev_dpdk_flow_del*/
...
netdev_init_flow_api(struct netdev *netdev)
{
	...
	/*流表操作api初始化*/
    return (class->init_flow_api ? class->init_flow_api(netdev) : EOPNOTSUPP);
}
...
struct port_to_netdev_data {
    ...
    struct netdev *netdev;									     	/*端口关联的netdev*/
    struct dpif_port dpif_port;
    const struct dpif_class *dpif_class;							/*dp操作函数*/
    int ifindex;
};
...
/*******************************************************************************
 函数名称  :    netdev_ports_lookup
 功能描述  :    根据端口号查询netdev data
 输入参数  :  	port_no---端口号
 				dpif_class---dp 接口类
 输出参数  :	
 返 回 值  : 	无
    ...
    OVS_REQUIRES(netdev_hmap_mutex)
{
	...
	/*遍历端口netdev 匹配类和端口号*/
    HMAP_FOR_EACH_WITH_HASH (data, portno_node, netdev_ports_hash(port_no, dpif_class), &port_to_netdev) 
    {
    	/*端口 netdev data*/
        if (data->dpif_class == dpif_class && data->dpif_port.port_no == port_no) 
        {
            return data;
        }
    }
...
/*******************************************************************************
 函数名称  :    netdev_ports_insert
 功能描述  :    netdev端口插入到全局端口netdev管理结构、index插入到全局index结构ifindex_to_port
 输入参数  :  	netdev---端口netdev
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
...
*******************************************************************************/
int
netdev_ports_insert(struct netdev *netdev, const struct dpif_class *dpif_class, struct dpif_port *dpif_port)
{
    struct port_to_netdev_data *data;
	...
	/*port数据插入到全局端口netdev管理结构port_to_netdev*/
    hmap_insert(&port_to_netdev, &data->portno_node, netdev_ports_hash(dpif_port->port_no, dpif_class));

	/*port数据插入全局端口index管理结构ifindex_to_port*/
	hmap_insert(&ifindex_to_port, &data->ifindex_node, ifindex);

	VLOG_DBG("netdev_ports_insert to ifindex_to_port port_to_netdev");
...
/*******************************************************************************
 函数名称  :    netdev_set_flow_api_enabled
 功能描述  :    使能api
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
netdev_set_flow_api_enabled(const struct smap *ovs_other_config)
{
    ...
    if (smap_get_bool(ovs_other_config, "hw-offload", false)) 
	{
        ...
        if (ovsthread_once_start(&once)) 
		{
            ...
            tc_set_policy(smap_get_def(ovs_other_config, "tc-policy", TC_POLICY_DEFAULT));
