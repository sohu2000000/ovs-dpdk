 ...
 * 'name' and 'type' are suitable for passing to netdev_open(). */
 /*端口信息*/
struct dpif_port {
    char *name;                 /* Network device name, e.g. "eth0". */			/*端口名*/
    char *type;                 /* Network device type, e.g. "system". */		/*端口类型*/
    odp_port_t port_no;         /* Port number within datapath. */				/*端口号*/
};
...
struct dpif_port_dump {
	/*数据接口类*/
    const struct dpif *dpif;	/*数据面操作函数*/
    int error;
    void *state;				/*port的状态*/
};
...
struct dpif_flow_stats {
    uint64_t n_packets;			/*流表命中报文数*/
    uint64_t n_bytes;			/*流表命中报文字节数*/
    long long int used;			/*流表是否命中过*/
    uint16_t tcp_flags;			
};
...
enum dpif_flow_put_flags {
    DPIF_FP_CREATE = 1 << 0,    /* Allow creating a new flow. */					/*添加新流表*/
    DPIF_FP_MODIFY = 1 << 1,    /* Allow modifying an existing flow. */				/*mod流表*/
    DPIF_FP_ZERO_STATS = 1 << 2, /* Zero the stats of an existing flow. */			/*清空流表流量统计*/
    DPIF_FP_PROBE = 1 << 3      /* Suppress error messages, if any. */				/*镇压错误消息*/
};
...
struct dpif_flow_put {
    /* Input. */
    enum dpif_flow_put_flags flags; /* DPIF_FP_*. */							/*dpctl流表动作宏*/
    const struct nlattr *key;       /* Flow to put. */							/*match描述符*/
    size_t key_len;                 /* Length of 'key' in bytes. */				/*所有match条件字节数*/
    const struct nlattr *mask;      /* Mask to put. */							/*match mask描述符*/
    size_t mask_len;                /* Length of 'mask' in bytes. */			/*mask条件字节数*/
    const struct nlattr *actions;   /* Actions to perform on flow. */			/*action描述符*/
    size_t actions_len;             /* Length of 'actions' in bytes. */			/*action的字节长度*/
    const ovs_u128 *ufid;           /* Optional unique flow identifier. */		/*流表ufid*/
    unsigned pmd_id;                /* Datapath poll mode driver id. */			/*流表下到的pmd*/

    /* Output. */
    struct dpif_flow_stats *stats;  /* Optional flow statistics. */			/*命中flow的流量统计*/
};
...
struct dpif_flow_del {
    ...
    /* Output. */
    struct dpif_flow_stats *stats;  /* Optional flow statistics. */						/*dpif 删除flow流量统计*/
};
...
struct dpif_execute {
    ...
    const struct flow *flow;         /* Flow extracted from 'packet'. */						/*flow*/

    /* Input, but possibly modified as a side effect of execution. */
    struct dp_packet *packet;          /* Packet to execute. */								/*要处理的报文*/
};
...
/*接口op*/
struct dpif_op {
    enum dpif_op_type type;						/*流表操作宏*/					
    int error;
    union {
        struct dpif_flow_put flow_put;			/*下发流表流表结构*/
