...
/*******************************************************************************
 函数名称  :    pmd_perf_format_overall_stats
 功能描述  :    emc流表删除
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称 :  pmd_perf_end_iteration
 功能描述 :  报文处理入口
 输入参数 :  s---pmd流量统计
 			 rx_packets---所有队列poll到的报文数
 			 tx_packets---发出的报文数
 输出参数 :  无
 返回值　 :  无
 --------------------------------------------------------------------------------
...
*******************************************************************************/
void
pmd_perf_end_iteration(struct pmd_perf_stats *s, int rx_packets, int tx_packets, bool full_metrics)
{
	/*最新的时间戳*/
    ...
    if (s->current.batches > 0) {
        histogram_add_sample(&s->pkts_per_batch, rx_packets / s->current.batches);
    }
    histogram_add_sample(&s->upcalls, s->current.upcalls);

	if (s->current.upcalls > 0) 
	{
        histogram_add_sample(&s->cycles_per_upcall, s->current.upcall_cycles / s->current.upcalls);
    }
