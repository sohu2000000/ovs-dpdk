...
/*******************************************************************************
 函数名称  :    json_string_create_nocopy
 功能描述  :    
 输入参数  :  	s---argv[i]，对应 "dpctl/add-flow" "flow"
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
...
json_string_create_nocopy(char *s)
{
	...
	/*指向具体的argv[i] 字符串，"dpctl/add-flow" "flow" */
    json->string = s;
    return json;
}
...
/*******************************************************************************
 函数名称  :    json_array_create
 功能描述  :    创建json数组，描述argv[i]
 输入参数  :  	elements---jason指针数组，指向jason结构，"dpcl/add-flow, "flow", 每个串对应一个jason，string指向argv[]
  				n----2个参数
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
...
json_integer_create(long long int integer)
{
	...
	VLOG_DBG("create json->integer=%Ld",json->integer)
    return json;
}
