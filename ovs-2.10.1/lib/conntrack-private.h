...
/*ct的链接*/
struct conn {
    struct conn_key key;											/*send tuple*/
    struct conn_key rev_key;										/*recv tuple*/
    /* Only used for orig_tuple support. */
    struct conn_key master_key;
    long long expiration;											/*超时时间*/
    struct ovs_list exp_node;										/*超时链表头*/
    struct hmap_node node;
    ovs_u128 label;
    /* XXX: consider flattening. */
    struct nat_action_info_t *nat_info;								/*nat*/
    char *alg;
    int seq_skew;
    uint32_t mark;
    uint8_t conn_type;
