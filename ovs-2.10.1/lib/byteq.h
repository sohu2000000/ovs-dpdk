...
struct byteq {
    uint8_t *buffer;            /* Circular queue. */								/*512字节环形队列buffer*/
    unsigned int size;          /* Number of bytes allocated for 'buffer'. */		/*队列长度512字节*/
    unsigned int head;          /* Head of queue. */								/*头index*/
    unsigned int tail;          /* Chases the head. */								/*尾巴index*/
};
