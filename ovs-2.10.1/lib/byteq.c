...
/*******************************************************************************
 ...
 功能描述  :    字节管理队列初始化
 输入参数  :  	q---字节buffer管理队列
 				buffer---buffer地址 512字节的buffer
 				size---buffer的size，512字节
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
...
{
	/*512字节剩余的长度，size-(head-tail),head-tail=当前使用的字节*/
    int avail = byteq_avail(q);

	/*head到队列尾部的长度，最大可用的字节，因为是个buffer，一次只能使用到end*/
    int head_to_end = q->size - (q->head & (q->size - 1));
    return MIN(avail, head_to_end);
}

/*******************************************************************************
 函数名称  :    byteq_advance_head
 功能描述  :    更新head指向的位置
 输入参数  :  	q---512字节管理队列
 				n---新加入的n字节
 输出参数  :	
