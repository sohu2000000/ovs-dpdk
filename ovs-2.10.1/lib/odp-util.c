...
*******************************************************************************/
static int
parse_action_list(const char *s, const struct simap *port_names, struct ofpbuf *actions)
{
    int n = 0;
...
/*******************************************************************************
 函数名称  :  parse_odp_action
 功能描述  :  pmd发送端口的报文
 输入参数  :  s---flow字符串
 			  actions---解析出的action、填入actions
...
*******************************************************************************/
static int
parse_odp_action(const char *s, const struct simap *port_names, struct ofpbuf *actions)
{
	/*output 端口*/
    {
        uint32_t port;
	...
	/*trunc动作*/
    {
        ...
        if (ovs_scan(s, "trunc(%"SCNi32")%n", &max_len, &n)) {
            ...
            trunc = nl_msg_put_unspec_uninit(actions, OVS_ACTION_ATTR_TRUNC, sizeof *trunc);
...
/*******************************************************************************
 函数名称  :    odp_actions_from_string
 功能描述  :    action提取
 输入参数  :  	port_names--参数
 				actions---action提取后赋值给actions
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
*******************************************************************************/
int
odp_actions_from_string(const char *s, const struct simap *port_names, struct ofpbuf *actions)
{
    size_t old_size;
...
/*******************************************************************************
 函数名称  :    odp_ufid_from_string
 功能描述  :    flow参数里提取flow ufid
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 ...
 功能描述  :	emc流表删除
 输入参数  :	NAME---match字段启始字符串 "skb_priority("
				TYPE---uint32 解析成的字段的类型
 输出参数  :	
 返 回 值  :	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
/*******************************************************************************
 ...
 输入参数  :  	NAME---match字段启始字符串 "skb_priority("
 				TYPE---uint32 解析成的字段的类型
 				SCAN_AS---u32 字段的扫描类型
 				ATTR---u32 skb->priority 提取的参数要赋值的变量
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
/*******************************************************************************
 函数名称  :	parse_odp_nsh_key_mask_attr
 功能描述  :	match条件解析完填到match 和match mask
 输入参数  :	
 输出参数  :	
 返 回 值  :	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    parse_odp_key_mask_attr
 功能描述  :    flow各字段提取
 输入参数  :  	s---flow match 字段指针数组
 				key---match 内存
 			    mask---match mask 内存
                        ...
                        struct ofpbuf *key, struct ofpbuf *mask)
{
	...
	/*match解完赋值入match的 key、和 match的mask*/
...
/*******************************************************************************
 函数名称  :    odp_flow_from_string
 功能描述  :    flow各字段解析提取，填入key、mask 
 输入参数  :  	s---match参数数组
 				port_names---提取的dp上的端口 hmap
 				key---match 内存
 				mask---match对应mask 内存
...
*******************************************************************************/
int
odp_flow_from_string(const char *s, const struct simap *port_names, struct ofpbuf *key, struct ofpbuf *mask)
{
	/*初始为match申请的内存size*/
    ...
    for (;;) 
	{
		...
		/*flow各字段解析提取，from s 参数数组 填入key、mask 具体没看出怎么填的*/
        retval = parse_odp_key_mask_attr(s, port_names, key, mask);
        if (retval < 0) 
		{
            key->size = old_size;
...
/*******************************************************************************
 函数名称  :    parse_flow_nlattrs
 功能描述  :    解析流表match条件
 输入参数  :  	key---match描述符
 				key_len---match条件字节数
 				attrs---match条件属性指针数值
 				flow---flow结构，解析出的match 填充到match的flow
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 最近一次修改记录 :
...
parse_flow_nlattrs(const struct nlattr *key, size_t key_len,
                   const struct nlattr *attrs[], uint64_t *present_attrsp, int *out_of_range_attrp)
{
    ...
    *out_of_range_attrp = 0;
    ...
    NL_ATTR_FOR_EACH (nla, left, key, key_len) 
	{
		...
		/*关键字长度*/
        int expected_len = odp_key_attr_len(ovs_flow_key_attr_lens, OVS_KEY_ATTR_MAX, type);
        ...
        if (len != expected_len && expected_len >= 0) 
		{
            ...
            VLOG_ERR_RL(&rl, "attribute %s has length %"PRIuSIZE" but should have "
                        "length %d", ovs_key_attr_to_string(type, namebuf, sizeof namebuf), len, expected_len);
            return false;
        }
...
/*******************************************************************************
 函数名称  :    odp_flow_key_to_flow__
 功能描述  :    提取key match条件各个字段赋值给flow
 输入参数  :  	key---match描述符
 				key_len---match条件字节数
 				flow---flow结构，解析出的match 填充到match的flow
 输出参数  :	
...
*******************************************************************************/
static enum odp_key_fitness
odp_flow_key_to_flow__(const struct nlattr *key, size_t key_len, struct flow *flow, const struct flow *src_flow)
{
	/*记录match的各个字段*/
    ...
    if (present_attrs & (UINT64_C(1) << OVS_KEY_ATTR_TUNNEL)) {
		...
		/*tunnel key*/
        res = odp_tun_key_from_attr__(attrs[OVS_KEY_ATTR_TUNNEL], is_mask, &flow->tunnel);
        if (res == ODP_FIT_ERROR) {
            return ODP_FIT_ERROR;
