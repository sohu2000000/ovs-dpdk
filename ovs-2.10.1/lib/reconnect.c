...
struct reconnect {
    ...
    int min_backoff;
    int max_backoff;
    int probe_interval;			/*探测时间间隔，默认是5000ms*/
    bool passive;
    enum vlog_level info;       /* Used for informational messages. */

    /* State. */
    enum state state;					/*fsm的状态*/
...
/*******************************************************************************
 函数名称  :    reconnect_create
 功能描述  :    创建一个reconnect fsm结构
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    reconnect_get_probe_interval
 功能描述  :    返回探活间隔毫秒级，设置0关掉探活
 				设置非0，fsm已建立链接但没有调用reconnect_activity()去重设定时器，调用reconnect_run() 返回 RECONNECT_PROBE去探测链接
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    reconnect_set_probe_interval
 功能描述  :    设置至少1000ms
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    reconnect_connected
 功能描述  :    重连，fsm间隔性探测，如果定时器超时，发一个prob，如果发完又超时，则链接aborted的
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
...
/*******************************************************************************
 函数名称  :    reconnect_activity
 功能描述  :    如果链接上有活动发生，重设探活定时器，链接不会空闲
 输入参数  :  	
 输出参数  :	
 返 回 值  : 	无
--------------------------------------------------------------------------------
 ...
 * one of:
 *
 *     - 0: The client need not take any action.     客户端不需要有action
 *
 *     - Active client, RECONNECT_CONNECT: The client should start a connection		客户端需要重连
 ...
 *
 *     - Passive client, RECONNECT_CONNECT: The client should try to listen for     客户端需要监听一个链接
 ...
 *
 *       A listening passive client should constantly attempt to accept a new       也支持接受链接的客户端
 ...
 *       reconnect_disconnected() or reconnect_connect_failed() to indicate it.     客户端需要断掉链接
 *
 ...
 *       interval" is nonzero--see reconnect_set_probe_interval()).                  客户端发echo request 探活链接，prob功能需要是开启的状态
 */
