...
/*数据面元数据metadata*/
struct pkt_metadata {
    //PADDED_MEMBERS_CACHELINE_MARKER(CACHE_LINE_SIZE, cacheline0,						/*为了看代码手动注释掉了*/
    uint32_t recirc_id;         /* Recirculation id carried with the				/*recircle id
                                   recirculating packets. 0 for packets
                                   received from the wire. */
    uint32_t dp_hash;           /* hash value computed by the recirculation			/*metadata hash值
                                   action. */
    uint32_t skb_priority;      /* Packet priority for QoS. */						//
    uint32_t pkt_mark;          /* Packet mark. */									/*mark值*/
    uint8_t  ct_state;          /* Connection state. */							    /*ct状态*/
    bool ct_orig_tuple_ipv6;
    uint16_t ct_zone;           /* Connection zone. */								/*ct zone*/
    uint32_t ct_mark;           /* Connection mark. */								
    ovs_u128 ct_label;          /* Connection label. */
    union flow_in_port in_port; /* Input port. */									/*in_port*/
);
...
/*******************************************************************************
 函数名称  :    pkt_metadata_init
 功能描述  :    vxlan报文元数据初始化，报文没有metadata，赋值
 输入参数  :    md---报文metadata
 			    port---vxlan报文发端口号
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称 :  pkt_metadata_prefetch_init
 功能描述 :  报文metadata预取处理
 输入参数 :  
 输出参数 :  无
 返回值　 :  无
