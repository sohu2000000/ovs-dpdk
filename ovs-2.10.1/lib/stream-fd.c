...
struct stream_fd
{
    struct stream stream;		/*具体的流*/
    int fd;						/*打开的/usr/local/var/run/openvswitch/ovs-vswtichd.pid.ctl的fd*/
    int fd_type;				/*unix socket文件链接状态*/
};
...
/*******************************************************************************
 函数名称  :    new_fd_stream
 功能描述  :    创建一个stream结构
 输入参数  :  	name---文件路径 unix:/usr/local/var/run/openvswitch/ovs-vswtichd.pid.ctl
 				fd---打开的unix_socket链接fd
 				streamp---赋值创建的stream
 				fd_type---链接状态
 				dscp---DSCP_DEFAULT
 输出参数  :	
...
fd_connect(struct stream *stream)
{
	...
	/*检查/usr/local/var/run/openvswitch/ovs-vswtichd.pid.ctl链接状态*/
...
struct fd_pstream
{
    ...
    int fd;
    int (*accept_cb)(int fd, const struct sockaddr_storage *, size_t ss_len,			/*punix_accept*/
                     struct stream **);
    char *unlink_path;
};
...
/*******************************************************************************
 函数名称  :    pfd_accept
 功能描述  :    接收函数
 输入参数  :  	new_streamp---存储accept后产生的新的链接
 输出参数  :	
 返 回 值  : 	无
-----------------------------------------------------------------------------
---
...
pfd_accept(struct pstream *pstream, struct stream **new_streamp)
{
	...
	/*接收链接后的回调函数 punix_accept，接收一个链接请求，产生新的链接赋值给new_streamp*/
