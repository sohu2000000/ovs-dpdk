...
struct vport_class {
    ...
    struct netdev_class netdev_class;  /*设备类结构*/
};
...
/*******************************************************************************
 函数名称  :    netdev_vport_needs_dst_port
 功能描述  :    网桥虚拟端口需要目的端口
 输入参数  :  	dev---网络设备结构
 输出参数  :	
 返 回 值  : 	无
...
netdev_vport_needs_dst_port(const struct netdev *dev)
{
	...
	/*获取netdev 类型 "system", "tap", "gre"*/
    const char *type = netdev_get_type(dev);

	/*获取数据面隧道方式类型 geneve vxlan lisp stt*/
...
/*******************************************************************************
 函数名称  :    netdev_vport_class_get_dpif_port
 功能描述  :    获取数据面端口
 输入参数  :  	class---设备类结构
 输出参数  :	
 返 回 值  : 	无
...
/*******************************************************************************
 函数名称  :    netdev_vport_get_dpif_port
 功能描述  :    虚拟端口获取dpif_port
 输入参数  :  	netdev---dp 网络设备结构
 				namebuf---缓存name
...
*******************************************************************************/
const char *
netdev_vport_get_dpif_port(const struct netdev *netdev, char namebuf[], size_t bufsize)
{
	/*获取dp设备类结构*/
    const struct netdev_class *class = netdev_get_class(netdev);
	...
	/*网桥虚拟端口需要目的端口 geneve vxlan lisp stt 等类型*/
    if (netdev_vport_needs_dst_port(netdev)) 
	{
		/*netdev获取netdev下vport*/
		...
		/*构造端口类型*/
        snprintf(namebuf, bufsize, "%s_%d", dpif_port, ntohs(vport->tnl_cfg.dst_port));
