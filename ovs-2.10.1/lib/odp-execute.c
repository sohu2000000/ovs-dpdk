...
/*******************************************************************************
 函数名称  :  requires_datapath_assistance
 功能描述  :  pmd发送端口的报文
 输入参数  :  
 ...
 * then the packet could either be cloned or not. */
 ...
 功能描述  :  pmd发送端口的报文
 输入参数  :  dp---pmd线程和要处理的flow 
 			  packets--每流报文批处理数组
 			  should_steal---是否steal
 			  actions---流表的action
 			  actions_len---流对应的action个数
 			  dp_execute_action---action 回调函数
...
*******************************************************************************/
void
odp_execute_actions(void *dp, struct dp_packet_batch *batch, bool steal, const struct nlattr *actions, size_t actions_len, odp_execute_cb dp_execute_action)
{
	/*批处理报文结构*/
    ...
    NL_ATTR_FOR_EACH_UNSAFE (a, left, actions, actions_len) 
	{
        ...
        switch ((enum ovs_action_attr) type) 
		{
	        ...
	        case OVS_ACTION_ATTR_SAMPLE:
	            DP_PACKET_BATCH_FOR_EACH (i, packet, batch) 
				{
	                odp_execute_sample(dp, packet, steal && last_action, a, dp_execute_action);
	            }
	        ...
	        case OVS_ACTION_ATTR_TRUNC: 
			{
	            const struct ovs_action_trunc *trunc = nl_attr_get_unspec(a, sizeof *trunc);
