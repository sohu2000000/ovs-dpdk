...
struct flowmap {
	...
	/*units = sizeof(struct flow)/8=552/8=69  ËùÒÔÐèÒª2¸öu64*/
    map_t bits[FLOWMAP_UNITS];   /*typedef unsigned long long, flow/8×Ö½Ú=8×Ö½Ú³¤¶È¸öÊý*/
};
...
/*¼ÇÂ¼Ò»¸öflowµÄÐÅÏ¢µÄÎ»Í¼,°üÍ·½«±»ÌáÈ¡´æÈëminiflow
miniflowÊÇstruct flowµÄÏ¡Êè±íÊ¾
1.¼õÉÙÄÚ´æÒÔ¼°¸ßËÙ»º´æÇø¿é£¨cache lines£©
2.ÓÉÓÚstruct flow½á¹¹·Ç³£´ó²¢ÇÒ´ó¶àÊýÖµÎª0£¬Ê¹ÓÃminiflow¿É¿ìËÙ¶Ô·Ç0Öµ½øÐÐµü´ú£¬
Ã¿¸östruct flowÖÐµÄuint64_tÔÚminiflow.map.bitsÖÐÕ¼1Î»*/
struct miniflow {	
    struct flowmap map;					/*flow ×Ö¶ÎÐÅÏ¢Î»Í¼*/
    /* Followed by:
     *     uint64_t values[n];
...
/*******************************************************************************
 º¯ÊýÃû³Æ  :    miniflow_values
 ¹¦ÄÜÃèÊö  :    miniflowµÄÖµ
 ÊäÈë²ÎÊý  :  	mf---miniflow
 Êä³ö²ÎÊý  :	
 ·µ »Ø Öµ  : 	ÎÞ
--------------------------------------------------------------------------------
 ×î½üÒ»´ÎÐÞ¸Ä¼ÇÂ¼ :
...
/*******************************************************************************
 º¯ÊýÃû³Æ  :    miniflow_get_values
 ¹¦ÄÜÃèÊö  :    mask miniflow
 ÊäÈë²ÎÊý  :  	
 Êä³ö²ÎÊý  :	
 ·µ »Ø Öµ  : 	ÎÞ
--------------------------------------------------------------------------------
...
/* Get the data from â€˜aux->valuesâ€™ corresponding to the next lowest 1-bit
 * in â€˜aux->mapâ€™, given that â€˜aux->valuesâ€™ points to an array of 64-bit
 * words corresponding to the 1-bits in â€˜aux->fmapâ€™, starting from the
 * rightmost 1-bit.
 *
 * Returns â€™trueâ€™ if the traversal is incomplete, â€˜falseâ€™ otherwise.
 * â€˜auxâ€™ is prepared for the next iteration after each call.
 *
                   ...
                   uint64_t *value)
{
    ...
    *map -= rm1bit;
    ...
    /* If the rightmost 1-bit found from the current unit in â€˜aux->mapâ€™
     * (â€˜rm1bitâ€™) is also present in â€˜aux->fmapâ€™, store the corresponding
     * value from â€˜aux->valuesâ€™ to â€˜*value', otherwise store 0. */
    if (OVS_LIKELY(*fmap & rm1bit)) {
        /* Skip all 64-bit words in â€˜valuesâ€™ preceding the one corresponding
         * to â€˜rm1bitâ€™. */
...
/*******************************************************************************
 º¯ÊýÃû³Æ  :    pkt_metadata_from_flow
 ¹¦ÄÜÃèÊö  :    ´Óflow»ñÈ¡metadata
 ÊäÈë²ÎÊý  :  	
 Êä³ö²ÎÊý  :	
 ·µ »Ø Öµ  : 	ÎÞ
--------------------------------------------------------------------------------
