...
/*******************************************************************************
 函数名称  :    fork_and_wait_for_startup
 功能描述  :    父进程一直等待子进程的完成信号，当收到子进程的信号后，文件描述符(*fdp)
...
/*******************************************************************************
 函数名称  :    启动系统守护进程
 功能描述  :    如果系统守护进程被配置了，启动系统守护进程，通过派生和在返回的子进程。父进程徘徊，直到
...
daemonize_start(bool access_datapath)
{
	...
	/* 根据命令行参数（命令行参数设置detach），则全局变量detach是1 因此会进入if分支。*/
    if (detach) {
		...
		/* 进程A(PID=1005)一直阻塞在管道那里，等待数据可读。
		当管道有可读数据时候，进程A就会跳出阻塞状态，然后进入if分支，最后进程A(PID=1005)就结束了。 */
        if (pid > 0) {
            /* Running in parent process. */
            exit(0);
        }
    ...
    if (monitor) {/* 根据命令行参数可知，全局变量monitor是1 */
        ...
        if (daemon_pid > 0) {
			...
			/* 在管道写入数据以便通知原始的父进程—进程A，这样进程A就会退出 */
