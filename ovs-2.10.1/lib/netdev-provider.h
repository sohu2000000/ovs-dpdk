...
struct netdev 
{
    ...
    const struct netdev_class *netdev_class; /* Functions to control				/*netdev操作类，dpdk类型为static const struct netdev_class dpdk_class*/
                                                this device. */
    ...
    uint64_t last_reconfigure_seq;		/*更新重新配置序列号*/
     ...
     * modify them. */
    int n_txq;							/*发送队列个数，默认1*/
    int n_rxq;                          /*默认1个txq*/
    struct shash_node *node;            /* Pointer to element in global map. */			/*全局链表节点*/
...
/*接收队列*/
struct netdev_rxq {
    struct netdev *netdev_zwl;      /* Owns a reference to the netdev. */	/*端口netdev*/
    int queue_id;               /*队列id*/
};
...
struct netdev_class {
     ...
     * the system. */
    const char *type;				/*网络设备的类型 "system", "tap", "gre"*/

    /* If 'true' then this netdev should be polled by PMD threads. */
    bool is_pmd;																	/*设备是否 pmd接管*/													
...
/* ## ---------------- ## */
    ...
    int (*construct)(struct netdev *);			/*虚拟端口构造类函数*/
    ...
    int (*set_config)(struct netdev *netdev, const struct smap *args,								/*配置端口*/
                      char **errp);
 ...
 * function pointers must be NULL. */
     ...
     * Return 0 if successful, otherwise returns a positive errno value. */
    int (*flow_put)(struct netdev *, struct match *, struct nlattr *actions,		/*netdev_dpdk_flow_put*/
