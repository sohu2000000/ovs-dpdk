...
struct ip_ct_tcp {
	...
	u_int8_t	state;		/* state of the connection (enum tcp_conntrack) */			/*链接状态*/
	/* For detecting stale connections */
	u_int8_t	last_dir;	/* Direction of the last packet (enum ip_conntrack_dir) */	/*最后一个包的方向*/
	u_int8_t	retrans;	/* Number of retransmitted packets */						/*重发的包数*/
	u_int8_t	last_index;	/* Index of the last packet */								/*最后一个包的index*/
	u_int32_t	last_seq;	/* Last sequence number seen in dir */						/*本方向最后一个报文序列号*/
	u_int32_t	last_ack;	/* Last sequence number seen in opposite dir */				/*反方向最后一个报文序列号*/
	u_int32_t	last_end;	/* Last seq + len */									    /*最后一个序列号加len*/
	u_int16_t	last_win;	/* Last window advertisement seen in dir */					/*最后一个方向的窗口广告*/
