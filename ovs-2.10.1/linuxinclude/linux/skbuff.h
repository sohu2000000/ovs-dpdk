...
struct sk_buff {
	union {
		...
		struct {
			/* These two members must be first. */
			/*这两个域是用来连接相关的skb的(例如如果有分片，将这些分片连接在一起可以)*/
	...
	struct sock		*sk;	/*指向报文所属的套接字指针*///指针，指向拥有此缓冲区套接字的sock数据结构。当缓冲区只是转发则不需要设置为NULL.

	union {
		ktime_t		tstamp;	/*记录接收或者传输报文的时间戳*///分组到达或离开的时间
		u64		skb_mstamp;
	};
	/*
...
#endif
#if defined(CONFIG_NF_CONNTRACK) || defined(CONFIG_NF_CONNTRACK_MODULE)
	unsigned long		 _nfct;											/*skb 关联的nf conntrack状态标记*/
#endif
#if IS_ENABLED(CONFIG_BRIDGE_NETFILTER)
...
#define PKT_TYPE_OFFSET()	offsetof(struct sk_buff, __pkt_type_offset)
	...
	__u8			pkt_type:3;// 定义在include/uapi/linux/if_packet.h,共３位最多８个值。
	__u8			pfmemalloc:1;
