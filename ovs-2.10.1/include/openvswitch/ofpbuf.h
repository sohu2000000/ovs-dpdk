...
/*存储任意数据的结构*/
struct ofpbuf {
    void *base;                 /* First byte of allocated space. */					/*内存首地址*/
    void *data;                 /* First byte actually in use. */						/*使用的数据首地址*/
    uint32_t size;              /* Number of bytes in use. */							/*使用的数据字节数*/
    uint32_t allocated;         /* Number of bytes allocated. */						/*申请的内存数*/

    void *header;               /* OpenFlow header. */
    void *msg;                  /* message's body */									/*消息结构*/
    struct ovs_list list_node;  /* Private list element for use by owner. */			/*链表头*/
    enum ofpbuf_source source;  /* Source of memory allocated as 'base'. */				/*内存申请类型*/
};
