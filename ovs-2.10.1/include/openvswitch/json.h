...
/* A JSON array. */
struct json_array {
    size_t n, n_allocated;			/*jason的个数*/
    struct json **elems;			/*jason指针数组，指向多个jason结构，每个jason对应一个argv[i]，string指向argv[i]*/
};

/*jaason的值*/
/* A JSON value. */
struct json {
    enum json_type type;												/*Jason的类型*/
    size_t count;														/*jason数据的个数*/
    union {
        struct shash *object;   /* Contains "struct json *"s. */		/*jason值链表*/
        struct json_array array;										/*Jason数组*/
        long long int integer;											/*整型数据*/
        double real;
        char *string;													/*具体的jason字符串 dpctl/add-flow*/
    };
};
